...
struct stasis_topic *stasis_topic_pool_get_topic(struct stasis_topic_pool *pool, const char *topic_name)
{
	 ...
	 * scope of the system we prefix it with the pooling topic name, which should
	...
	if (!topic_pool_entry->topic) {
		/*
		 * stasis_topic_create() can fail if another thread created
		 * the topic concurrently (topics have global unique names).
		 * Reacquire the pool lock and check if an entry now exists.
		 */
		ao2_lock(pool->pool_container);

		existing = ao2_find(pool->pool_container, topic_name, OBJ_SEARCH_KEY | OBJ_NOLOCK);
		if (existing) {
			/* Use the existing topic that won the race */
		...
		/* No existing entry - this is a genuine failure */
		ao2_ref(topic_pool_entry, -1);
		return NULL;
	}
