...
struct cnode
{
	...
	struct ast_channel *ptr;

	cnode(std::string _name, std::string _unique_id,
		struct ast_channel * ptr)
	:	name(_name), unique_id(_unique_id),
		ptr(ptr)
	{
	}
};
...
typedef multi_index_container<
	...
	indexed_by<
		ordered_unique<tag<ix_name>, BOOST_MULTI_INDEX_MEMBER(cnode,std::string,name)>,
		ordered_unique<tag<ix_unique_id>, BOOST_MULTI_INDEX_MEMBER(cnode,std::string,unique_id)>
		>
> ChannelMap;

struct bf_channelstorage_driver_pvt {
	ChannelMap by_name;
};

#define getdb(driver) (((struct bf_channelstorage_driver_pvt *)driver->handle)->by_name)
	...
	struct ast_channel *chan, int flags, int lock)
{
	...
	char *l_uniqueid = chan ? ast_str_to_lower(ast_strdupa(ast_channel_uniqueid(chan))) : NULL;
	if (!chan) {
		return -1;
	}
	...
	const auto rtn = getdb(driver).insert(
		std::make_shared<cnode>(l_name, l_uniqueid, ao2_bump(chan)));
	...
	struct ast_channel *chan, int lock)
{
	...
	auto& ix = getdb(driver).get<ix_name>();
	ix.erase(l_name);
...
struct bf_channel_iterator {
	...
	bf_channel_iterator(ChannelMap::const_iterator it, char *name, size_t name_len)
		: it(it), it_type(ITERATOR_BY_NAME), channel_name(name), channel_name_len(name_len),
		context(NULL), exten(NULL)
	{
	}

	bf_channel_iterator(ChannelMap::const_iterator it, char *context, char *exten)
		: it(it), it_type(ITERATOR_BY_EXTEN), channel_name(NULL), channel_name_len(0),
		context(context), exten(exten)
	{
	}

	bf_channel_iterator(ChannelMap::const_iterator it)
...
{
	delete (struct bf_channel_iterator *)i;
	return NULL;
}

static struct ast_channel_iterator *iterator_all_new(struct ast_channelstorage_instance *driver)
{
	struct bf_channel_iterator *i = new bf_channel_iterator(getdb(driver).begin());
	if (!i) {
		return NULL;
	}
...
{
	struct bf_channel_iterator *i = (struct bf_channel_iterator *)ai;
	struct ast_channel *chan = NULL;
	...
	while (i->it != getdb(driver).end()) {
		if (i->it_type == ITERATOR_BY_NAME) {
			chan = (struct ast_channel *)((*(i->it))->ptr);
			if (strncasecmp(ast_channel_name(chan), i->channel_name, i->channel_name_len) == 0) {
				++i->it;
				return ao2_bump(chan);
			}
		} else if (i->it_type == ITERATOR_BY_EXTEN) {
			int ret = channelstorage_exten_cb((*(i->it))->ptr, i->context, i->exten, 0);
			if (ret & CMP_MATCH) {
				chan = ao2_bump((*(i->it))->ptr);
				++i->it;
				return chan;
...
{
	struct bf_channel_iterator *i =
		new bf_channel_iterator(getdb(driver).begin(), ast_strdup(name), name_len);
	if (!i) {
		return NULL;
	}
...
{
	struct bf_channel_iterator *i =
		new bf_channel_iterator(getdb(driver).begin(),
...
static struct ast_channel *get_by_uniqueid(struct ast_channelstorage_instance *driver,
	const char *uniqueid)
{
	char *l_uniqueid = uniqueid ? ast_str_to_lower(ast_strdupa(uniqueid)) : NULL;
	if (ast_strlen_zero(uniqueid)) {
		return NULL;
	}
	auto& ix = getdb(driver).get<ix_unique_id>();
	auto chanx = ix.find(l_uniqueid);
	if (chanx == ix.end()) {
		return NULL;
	}
	return ao2_bump((*chanx)->ptr);
}
	...
	const char *name, size_t name_len)
{
	...
	auto& ix2 = getdb(driver).get<ix_name>();
	l_name[name_len] = '\0';
	ast_str_to_lower(l_name);
	auto rtn = ix2.lower_bound(l_name);
	if (rtn != ix2.end()) {
		chan = ao2_bump((*rtn)->ptr);
	}
...
static void close_instance(struct ast_channelstorage_instance *driver)
{
	...
	if (driver->handle) {
		delete ((struct bf_channelstorage_driver_pvt *)driver->handle);
		driver->handle = NULL;
	}
...
static struct ast_channelstorage_instance* get_instance(const char *name)
{
	...
	driver->handle = (struct ast_channelstorage_driver_pvt *)new bf_channelstorage_driver_pvt();
