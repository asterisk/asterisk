	...
	char *file_path, size_t len)
{
	 ...
	 * matching the requested URI, ask the appropriate backend to update
	 * the resource before we return it.
	 */
	...
	if (bucket_file) {
		...
		/* Check the backend to see if we need to update */
		ast_bucket_file_update(clone);
		bucket_file_update_path(clone, preferred_file_name);
		ast_copy_string(file_path, clone->path, len);
		media_cache_item_sync_to_astdb(clone);

		/* Replace the old bucket_file with the new updated clone */
		ao2_unlink_flags(media_cache, bucket_file, OBJ_NOLOCK);
		ao2_link_flags(media_cache, clone, OBJ_NOLOCK);
		ao2_ref(clone, -1);
		ao2_ref(bucket_file, -1);
	...
	/* We have never seen this URI before; do a full retrieve from the
	 * appropriate bucket_file backend
	 */
	bucket_file = ast_bucket_file_retrieve(uri);
	if (!bucket_file) {
