...
static void *test_storage_thread(void *data)
{
	...
	for (i = 0; i < CHANNEL_COUNT; i++) {
		...
		ast_test_validate_cleanup(test, mock_channel, res, done);
//		ast_test_status_update(test, "%s %s %s\n", mock_channel->name, mock_channel->context, mock_channel->exten);
		ast_channel_unref(mock_channel);
	}
	end = ast_tvnow();
	elapsed = ast_tvdiff_us(end, start);
	...
	for (i = 0; i < CHANNEL_COUNT; i++) {
		if (test_channels[i]) {
//			ast_test_status_update(test, "Deleting channel %s\n", ast_channel_name(test_channels[i]));
...
DEFINE_STORAGE_TEST(ao2_optimized)

DEFINE_STORAGE_TEST(sqlite3)

DEFINE_STORAGE_TEST(cpp_map_name)

DEFINE_STORAGE_TEST(cpp_map_name_id)

DEFINE_STORAGE_TEST(boost_map_name)

DEFINE_STORAGE_TEST(boost_multi_index)
...
static void channelstorage_shutdown(void)
{
	...
	AST_TEST_UNREGISTER(sqlite3);
	AST_TEST_UNREGISTER(boost_multi_index);
	AST_TEST_UNREGISTER(boost_map_name);
	AST_TEST_UNREGISTER(cpp_map_name_id);
	AST_TEST_UNREGISTER(cpp_map_name);
	AST_TEST_UNREGISTER(ao2_optimized);
...
int ast_channelstorage_init(void)
{
	...
	REGISTER_STORAGE_TEST(sqlite3);
	REGISTER_STORAGE_TEST(boost_multi_index);
	REGISTER_STORAGE_TEST(boost_map_name);
	REGISTER_STORAGE_TEST(cpp_map_name_id);
	REGISTER_STORAGE_TEST(cpp_map_name);
	REGISTER_STORAGE_TEST(ao2_optimized);
