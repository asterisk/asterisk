...
static void device_state_notify_callbacks(enum ast_event_type type, struct ast_hint *hint,
	...
	for (; (device = ao2_iterator_next(dev_iter)); ao2_t_ref(device, -1, "Next device")) {
		if (device->hint) {
			device_state_notify_callbacks(
				AST_EVENT_DEVICE_STATE, device->hint, &hint_app, 0);
		}
	}
	ast_mutex_unlock(&context_merge_lock);
...
static int publish_hint_change(struct ast_hint *hint, struct ast_exten *ne)
{
	...
	struct hint_change_payload *payload = hint_change_payload_create(hint, ne);
...
static int handle_hint_change(void *data)
{
	...
	device_state_notify_callbacks(
		AST_EVENT_HINT_CHANGE, payload->hint, &hint_app, payload->device_state);

	presence_state_notify_callbacks(
		AST_EVENT_HINT_CHANGE, payload->hint, &hint_app, &presence_state);
