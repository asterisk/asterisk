...
static struct ast_frame *__ast_read(struct ast_channel *chan, int dropaudio)
{
	...
	if (ast_test_flag(ast_channel_flags(chan), AST_FLAG_DEFER_FRAMES)) {
		...
		if (head
			...
			ast_debug(1, "Frame deferral: Already got a hangup on: %s\n",
				ast_channel_name(chan));
			...
			ast_debug(1, "Frame deferral: Got %s hangup NULL on: %s\n",
				dead ? "real" : "fake(i.e. async-goto)", ast_channel_name(chan));
			if (dead) {
				/* Channel is a zombie or a real hangup.  Defer it as a hangup. */
				static struct ast_frame hangup_frame = {
					.frametype = AST_FRAME_CONTROL,
					.subclass.integer = AST_CONTROL_HANGUP,
				};

				dup = ast_frdup(&hangup_frame);
				if (dup) {
					AST_LIST_INSERT_HEAD(ast_channel_deferred_readq(chan), dup, frame_list);
				}
			} else {
				/*
				 * The hangup isn't real but whoever is deferring frames
				 * needs to treat it as such and stop reading.
				 */
			}
		} else if (f->frametype == AST_FRAME_CONTROL
			&& f->subclass.integer == AST_CONTROL_HANGUP) {
			ast_debug(1, "Frame deferral: Got hangup frame on: %s\n",
				ast_channel_name(chan));
			...
			/* Report hangup event to deferrer. */
			ast_frfree(f);
			f = NULL;
