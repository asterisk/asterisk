		       ...
		       const char *function, const char *name_fmt, va_list ap)
{
	...
	if (!(nativeformats = ast_format_cap_alloc(AST_FORMAT_CAP_FLAG_DEFAULT))) {
		/*
		 * We do not need to complete the snapshot staging because the channel
		 * has not been finalized or linked into the channels container yet.
		 * Nobody knows about this channel nor will anybody ever know about it.
		 */
