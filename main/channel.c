...
{
	RAII_VAR(struct ast_json *, channel_event, NULL, ast_json_unref);
	char digit_str[] = { digit, '\0' };

	channel_event = ast_json_pack("{ s: s, s: s }",
		"digit", digit_str,
		"direction", dtmf_direction_to_string(direction));

	if (channel_event) {
		ast_channel_publish_blob(chan, ast_channel_dtmf_begin_type(), channel_event);
	}
}
...
{
	RAII_VAR(struct ast_json *, channel_event, NULL, ast_json_unref);
	RAII_VAR(struct ast_json *, cel_event, NULL, ast_json_unref);
	char digit_str[] = { digit, '\0' };

	channel_event = ast_json_pack("{ s: s, s: s, s: I }",
		"digit", digit_str,
		"direction", dtmf_direction_to_string(direction),
		"duration_ms", (ast_json_int_t)duration_ms);

	if (channel_event) {
		ast_channel_publish_blob(chan, ast_channel_dtmf_end_type(), channel_event);
	}

	cel_event = ast_json_pack("{ s: s, s: { s: s, s: I }}",
	...
	if (cel_event) {
		ast_cel_publish_event(chan, AST_CEL_DTMF, cel_event);
	} else {
		ast_log(LOG_WARNING, "Unable to build extradata for DTMF event on channel %s", ast_channel_name(chan));
	}
}
