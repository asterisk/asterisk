...
static void channel_do_masquerade(struct ast_channel *original, struct ast_channel *clonechan)
{
	...
	ast_debug(1, "Masquerading %s(%u) into the structure of %s(%u)\n",
...
int ast_channel_open_storage()
{
	if (!current_channel_storage_driver) {
		int rc = ast_channel_set_current_storage_driver(AST_CHANNELSTORAGE_DEFAULT_TYPE);
		if (rc) {
...
int ast_channel_set_current_storage_driver(const char *driver_name)
{
