...
#include "asterisk/shutdown_group.h"
 ...
 */
 static int taskpool_dynamic_pool_shrink(const void *data)
 {
	...
	return num_removed ? pool->options.idle_timeout * 1000 : -1;
 }
		...
		const struct ast_taskpool_options *options)
{
	...
	if (taskpool_taskprocessors_init(&pool->static_taskprocessors, options->initial_size)) {
		ao2_ref(pool, -1);
		return NULL;
	}

	/* We assume we will never have dynamic taskprocessors, so opt for a 0 initial size */
	if (taskpool_taskprocessors_init(&pool->dynamic_taskprocessors, 0)) {
		ao2_ref(pool, -1);
		return NULL;
	}

	/* Create the taskprocessors based on the passed-in options */
	for (int i = 0; i < options->initial_size; i++) {
		struct taskpool_taskprocessor *taskprocessor;
	...
	struct taskpool_taskprocessor **taskprocessor, unsigned int *growth_threshold_reached)
{
	...
	*growth_threshold_reached = (ast_taskprocessor_size((*taskprocessor)->taskprocessor) >= TASKPOOL_GROW_THRESHOLD) ? 1 : 0;
}
...
int ast_taskpool_push(struct ast_taskpool *pool, int (*task)(void *data), void *data)
{
	...
	if (!pool->shutting_down) {
		 ...
		 * it is present.
		 */
		taskpool_sequential_selector(pool, &pool->static_taskprocessors, &taskprocessor, &growth_threshold_reached);
		if (pool->options.auto_increment && growth_threshold_reached) {
			/* If we need to grow then try dynamic taskprocessors */
			taskpool_sequential_selector(pool, &pool->dynamic_taskprocessors, &taskprocessor, &growth_threshold_reached);
...
static int execute_tasks(void *data)
{
	 ...
	 * the serializer that it may be blocked on the the synchronous
