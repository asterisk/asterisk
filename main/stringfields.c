 ...
 * \retval -1 Failure
 */
static int __ast_string_field_free_memory(struct ast_string_field_mgr *mgr,
	...
	const char *file, int lineno, const char *func)
{
	...
	switch (cleanup_type) {
	case AST_STRINGFIELD_CLEANUP_FREE_POOLS:
		...
		mgr->header = NULL;
		break;
	case AST_STRINGFIELD_CLEANUP_RESET:
	...
	int needed, const char *file, int lineno, const char *func)
{
	...
	mgr->header->num_structs = 1;
	/* for clarity since we use these fields in __ast_string_field_free */
	mgr->header->size = 0;
	mgr->header->index = 0;
...
/*!
 * \brief Can free a single structure or an allocation returned from ast_calloc_with_stringfields
 * \internal
 *
 * \param initial_alloc
 * \param mgr
 ...
 * \retval -1 Failure
 */
int __ast_string_field_free(void *initial_alloc, struct ast_string_field_mgr *mgr,
	struct ast_string_field_pool **pool_head, enum ast_stringfield_cleanup_type cleanup_type,
	const char *file, int lineno, const char *func)
{
	int i;
	...
	callocd = !!mgr->header->embedded_pool;
	initial_index = mgr->header->index;
	size = mgr->header->size;
	num_structs = mgr->header->num_structs;

	for (i = 0; i < num_structs; i++) {
		size_t offset = size * i;
		struct ast_string_field_mgr *next_mgr = ((void *)mgr + offset);
		struct ast_string_field_pool **next_pool_head = ((void *)pool_head + offset);

		res += __ast_string_field_free_memory(next_mgr, next_pool_head,
			(cleanup_type == AST_STRINGFIELD_CLEANUP_FREE_ALL
				? AST_STRINGFIELD_CLEANUP_FREE_POOLS : cleanup_type),
			file, lineno, func);
	}

	if (cleanup_type == AST_STRINGFIELD_CLEANUP_FREE_ALL && callocd && initial_index == 0 && !res) {
		ast_free(initial_alloc);
	}
	...
	int lineno, const char *func)
{
	...
	for (x = 0; x < num_structs; x++) {
		...
		while ((struct ast_string_field_mgr *) p != mgr) {
			if (AST_VECTOR_APPEND(&mgr->header->string_fields, p)) {
				AST_VECTOR_FREE(&mgr->header->string_fields);
				ast_free(mgr->header);
				goto error;
			}
		...
		mgr->header->num_structs = num_structs;
		mgr->header->index = x;
		mgr->header->size = size_to_alloc;
...
error:
	if (x != num_structs) {
		...
		/* Roll it all back */
		for(y = 0; y < x; y++) {
			void *base = allocation + (size_to_alloc * y);
