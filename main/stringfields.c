	...
	int needed, const char *file, int lineno, const char *func)
{
	...
	/* This is cleanup only from here on */
	 ...
	 * operation.
	 */
	if ((needed < 0) && mgr->header->embedded_pool) {
		needed = 0;
	}
	...
	if (needed < 0) {		/* reset all pools */
		...
		AST_VECTOR_FREE(&mgr->header->string_fields);
		cur = *pool_head;
	} else if (mgr->header->embedded_pool) { /* preserve the embedded pool */
...
int __ast_string_fields_cmp(struct ast_string_field_vector *left, struct ast_string_field_vector *right)
{
	...
	if (AST_VECTOR_SIZE(left) != AST_VECTOR_SIZE(right)) {
		return -1;
	}
...
{
	int i;
	struct ast_string_field_vector *v1 = &(copy_mgr->header->string_fields);
	struct ast_string_field_vector *v2 = &(orig_mgr->header->string_fields);

	if (AST_VECTOR_SIZE(v1) != AST_VECTOR_SIZE(v2)) {
		return -1;
	}

	for (i = 0; i < AST_VECTOR_SIZE(v1); i++) {
		__ast_string_field_release_active(copy_pool, *AST_VECTOR_GET(v1, i));
		*AST_VECTOR_GET(v1, i) = __ast_string_field_empty;
	}
	for (i = 0; i < AST_VECTOR_SIZE(v1); i++) {
		if (ast_string_field_ptr_set_by_fields(copy_pool, *copy_mgr, AST_VECTOR_GET(v1, i), *AST_VECTOR_GET(v2, i))) {
			return -1;
		}
	}
