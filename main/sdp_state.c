...
void ast_sdp_state_free(struct ast_sdp_state *sdp_state)
{
	...
	ast_stream_topology_destroy(sdp_state->local_capabilities);
	ast_stream_topology_destroy(sdp_state->remote_capabilities);
	ast_stream_topology_destroy(sdp_state->joint_capabilities);
