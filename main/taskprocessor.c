...
struct tps_taskprocessor_stats {
	...
	/*! \brief Highest time (in milliseconds) spent processing a task */
	long highest_time_processed;
	/*! \brief Lowest time (in milliseconds) spent processing a task */
...
/*! \brief Flag to enable logging of task origin (file/line/function) */
static int log_task_origin = 0;
...
static char *cli_tps_task_debug(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a);
...
static struct ast_cli_entry taskprocessor_clis[] = {
	...
	AST_CLI_DEFINE(cli_tps_task_debug, "Enable/disable task origin debug logging"),
};
...
#define ast_taskprocessor_push_internal(tps, task_exe, datap) \
	ast_taskprocessor_push_origin(tps, task_exe, datap, __FILE__, __LINE__, __PRETTY_FUNCTION__)
int ast_taskprocessor_push(struct ast_taskprocessor *tps, int (*task_exe)(void *datap), void *datap);

#undef ast_taskprocessor_push_local
#define ast_taskprocessor_push_local_internal(tps, task_exe, datap) \
	ast_taskprocessor_push_local_origin(tps, task_exe, datap, __FILE__, __LINE__, __PRETTY_FUNCTION__)
...
static char *cli_tps_report(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a)
{
	...
	ast_cli(a->fd, "\n" FMT_HEADERS, "Processor", "Processed", "In Queue", "Max Depth", "Low water", "High water", "Low time", "High time");
...
static char *cli_tps_show_taskprocessor(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a)
{
	...
	ast_cli(a->fd, "Highest time (ms):    %ld\n", tps->stats.highest_time_processed);
...
static int taskprocessor_push(struct ast_taskprocessor *tps, struct tps_task *t)
{
	...
	if (log_task_origin && t->file) {
		ast_log(LOG_DEBUG, "Taskprocessor '%s': Task pushed from %s:%d (%s)\n",
	...
	if (tps->tps_queue_high <= tps->tps_queue_size) {
		if (!tps->high_water_alert) {
			ast_log(LOG_WARNING, "Taskprocessor '%s' queue reached %ld scheduled tasks (high water mark)%s.\n",
...
int ast_taskprocessor_push_origin(struct ast_taskprocessor *tps, int (*task_exe)(void *datap), void *datap
	, const char *file, int line, const char *function)
{
...
int ast_taskprocessor_push(struct ast_taskprocessor *tps, int (*task_exe)(void *datap), void *datap)
{
	return ast_taskprocessor_push_origin(tps, task_exe, datap, NULL, 0, NULL);
}

int ast_taskprocessor_push_local_origin(struct ast_taskprocessor *tps, int (*task_exe)(struct ast_taskprocessor_local *datap), void *datap
	, const char *file, int line, const char *function)
{
...
int ast_taskprocessor_push_local(struct ast_taskprocessor *tps, int (*task_exe)(struct ast_taskprocessor_local *datap), void *datap)
{
	return ast_taskprocessor_push_local_origin(tps, task_exe, datap, NULL, 0, NULL);
}
...
static char *cli_tps_task_debug(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a)
{
	switch (cmd) {
	case CLI_INIT:
		e->command = "core taskprocessor set debug";
		e->usage =
			"Usage: core taskprocessor set debug <on|off>\n"
			"    Enable or disable task origin debug logging.\n"
			"    When enabled, logs the file, line, and function where\n"
			"    each task is pushed into a taskprocessor.\n"
			"    Only available in DEVMODE builds.\n";
		return NULL;
	case CLI_GENERATE:
		if (a->pos == 4) {
	...
	if (!strcasecmp(a->argv[4], "on")) {
		log_task_origin = 1;
		ast_cli(a->fd, "Task origin debug logging enabled\n");
	} else if (!strcasecmp(a->argv[4], "off")) {
		log_task_origin = 0;
		ast_cli(a->fd, "Task origin debug logging disabled\n");
	} else {
		return CLI_SHOWUSAGE;
	}
