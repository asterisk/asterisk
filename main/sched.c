...
/*!
 * \brief Add new scheduler IDs to the stack.
 *
 * \retval The number of IDs added to the stack
 */
static int add_ids(struct ast_sched_context *con)
{
	...
	original_size = con->id_stack_size;
	 ...
	 */
	new_size = original_size + ID_STACK_INCREMENT;
	...
	return con->id_stack_size - original_size;
}

static int set_sched_id(struct ast_sched_context *con, struct sched *new_sched)
{
	if (AST_LIST_EMPTY(&con->id_stack) && (add_ids(con) == 0)) {
		return -1;
	}
