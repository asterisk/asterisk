...
static void session_instance_destructor(void *obj)
{
	...
	if (i->stream) {
		ao2_t_ref(i->stream, -1, "Destroying tcptls session instance");
		i->stream = NULL;
	}
...
#ifdef DO_SSL
		if (!ast_iostream_start_tls(&tcptls_session->stream, tcptls_session->parent->tls_cfg->ssl_ctx, tcptls_session->client)) {
			ast_tcptls_close_session_file(tcptls_session);
			ao2_ref(tcptls_session, -1);
			return NULL;
		}
...
struct ast_tcptls_session_instance *ast_tcptls_client_create(struct ast_tcptls_session_args *desc)
{
	...
	tcptls_session->stream = ast_iostream_from_fd(&desc->accept_fd);
	if (!tcptls_session->stream) {
		goto error;
	}
