...
void *ast_tcptls_server_root(void *data)
{
	...
	for (;;) {
		...
		memcpy(&tcptls_session->requestor, &sin, sizeof(tcptls_session->requestor));

		tcptls_session->client = 0;
			
		if (ast_pthread_create_detached_background(&launched, NULL, ast_make_file_from_fd, tcptls_session)) {
...
struct ast_tcptls_session_instance *ast_tcptls_client_start(struct server_args *desc)
{
	...
	memcpy(&tcptls_session->requestor, &desc->sin, sizeof(tcptls_session->requestor));
