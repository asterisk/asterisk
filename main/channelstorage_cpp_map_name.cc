...
struct mn_channelstorage_driver_pvt {
	ChannelMap by_name;
};

#define getdb(driver) (((struct mn_channelstorage_driver_pvt *)driver->handle)->by_name)
...
struct mn_channel_iterator {
	...
	mn_channel_iterator(ChannelMap::const_iterator it,
	...
	mn_channel_iterator(ChannelMap::const_iterator it, ChannelMap::const_iterator it_end)
...
{
	struct mn_channel_iterator *i = (struct mn_channel_iterator *)ai;
...
{
	struct mn_channel_iterator *i = new mn_channel_iterator(
	...
	const char *name, size_t name_len)
{
	...
	i = new mn_channel_iterator(getdb(driver).lower_bound(l_name),
...
{
	struct mn_channel_iterator *i =
		new mn_channel_iterator(getdb(driver).begin(),
			getdb(driver).end(),
...
static void close_instance(struct ast_channelstorage_instance *driver)
{
	...
	if (driver->handle) {
		delete (struct mn_channelstorage_driver_pvt *)driver->handle;
		driver->handle = NULL;
	}
...
static struct ast_channelstorage_instance* get_instance(const char *name)
{
	...
	driver->handle = (struct ast_channelstorage_driver_pvt *)new mn_channelstorage_driver_pvt();

	if (!driver->handle) {
		ast_log(LOG_ERROR, "Failed to channel storage driver %s\n",
			_name);
		ast_free(driver);
		return NULL;
	}
