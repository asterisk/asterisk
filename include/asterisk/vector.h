...
#define AST_RWVECTOR_SIZE(vec)  ({ \
	size_t __current = -1; \
	if (ast_rwlock_rdlock(&(vec)->lock) == 0) { \
		__current = (vec)->current; \
		ast_rwlock_unlock(&(vec)->lock); \
	} \
	__current; \
})
...
#define AST_RWVECTOR_GET(vec, idx) ({ \
	typeof((vec)->elems[0]) res = (typeof((vec)->elems[0]))NULL; \
	if (ast_rwlock_rdlock(&(vec)->lock) == 0) { \
		res = AST_VECTOR_GET(vec, idx); \
		ast_rwlock_unlock(&(vec)->lock); \
	} \
	res; \
})
 ...
 * was reached or CMP_STOP was returned.
 */
#define AST_RWVECTOR_CALLBACK_RDLOCK(vec, callback, ...) ({ \
	int res = -1; \
	if (ast_rwlock_rdlock(&(vec)->lock) == 0) { \
		res = AST_VECTOR_CALLBACK(vec, callback, ##__VA_ARGS__); \
		ast_rwlock_unlock(&(vec)->lock); \
	} \
	res; \
})
 ...
 * was reached or CMP_STOP was returned.
 */
#define AST_RWVECTOR_CALLBACK_WRLOCK(vec, callback, ...) ({ \
	int res = -1; \
	if (ast_rwlock_wrlock(&(vec)->lock) == 0) { \
		res = AST_VECTOR_CALLBACK(vec, callback, ##__VA_ARGS__); \
		ast_rwlock_unlock(&(vec)->lock); \
