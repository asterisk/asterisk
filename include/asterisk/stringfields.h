...
({ \
	int __res__ = -1; \
	if (((void *)(x)) != NULL) { \
		__res__ = __ast_string_field_free(x, &(x)->__field_mgr, &(x)->__field_mgr_pool, \
...
int __ast_string_field_free(void *initial_alloc, struct ast_string_field_mgr *mgr,
	struct ast_string_field_pool **pool_head, int cleanup_type, const char *file, int lineno,
	const char *func);
...
/*!
 * \brief Obtain the address of a structure in the allocation by index
 * \since 13.9.0
 *
 * \param x Pointer to a structure containing string fields
 * \param index The index of the desired structure
 * \return A pointer to the desired structure
 */
#define ast_calloc_with_stringfields_get_ptr(x, index) \
({ \
	typeof((x)) __x__ = NULL; \
	if (index < (x)->__field_mgr.header->num_structs) { \
		__x__ = ((void *)(x) + (x)->__field_mgr.header->size * index); \
	} \
	__x__; \
})

/*!
 * \brief Obtain the address of the next structure in the allocation
 * \since 13.9.0
 *
 * \param x Pointer to a structure containing string fields
 * \return A pointer to the next structure
 */
#define ast_calloc_with_stringfields_get_next_ptr(x) \
({ \
	typeof((x)) __x__ = NULL; \
	if ((x)->__field_mgr.header->index < (x)->__field_mgr.header->num_structs - 1) { \
		__x__ = ((void *)(x) + (x)->__field_mgr.header->size); \
	} \
	__x__; \
})
