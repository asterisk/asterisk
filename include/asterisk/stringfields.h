...
/*! \file
  ...
  * variants of these function allow passing a pointer to the field
  ...
  below sample below, it's not necessary for extended string fields.
  ...
  ast_string_field_init_extended(x, e);
  \endcode
...
({ \
	int __res__ = -1; \
	if (((void *)(x)) != NULL) { \
		__res__ = __ast_string_field_init(&(x)->__field_mgr, &(x)->__field_mgr_pool, -1, __FILE__, __LINE__, __PRETTY_FUNCTION__); \
	} \
	__res__ ; \
})
...
/*!
  ...
  \param x Pointer to a structure containing field
...
({ \
	...
	if (((void *)(x)) != NULL && (x)->__field_mgr.header != NULL) { \
		...
		__res__ = AST_VECTOR_APPEND(&(x)->__field_mgr.header->string_fields, &(x)->field); \
	} \
	__res__; \
})
...
({ \
	...
	int __i__; \
	int __len__; \
	struct ast_string_field_vector *__v1__ = NULL; \
	struct ast_string_field_vector *__v2__ = NULL; \
	if (((void *)(instance1)) != NULL && ((void *)(instance2)) != NULL) { \
		__v1__ = &((instance1)->__field_mgr.header->string_fields); \
		__v2__ = &((instance2)->__field_mgr.header->string_fields); \
		__len__ = AST_VECTOR_SIZE(__v1__); \
		if (__len__ == AST_VECTOR_SIZE(__v2__)) { \
			for (__i__ = 0; __i__ < __len__; __i__++) { \
				__res__ = strcmp(*AST_VECTOR_GET(__v1__, __i__), *AST_VECTOR_GET(__v2__, __i__)); \
				if (__res__) { \
					break; \
				} \
			} \
...
({ \
	...
	if (((void *)(copy)) != NULL && ((void *)(orig)) != NULL) { \
		__v1__ = &((copy)->__field_mgr.header->string_fields); \
		__v2__ = &((orig)->__field_mgr.header->string_fields); \
		__len__ = AST_VECTOR_SIZE(__v1__); \
		if (__len__ == AST_VECTOR_SIZE(__v2__)) { \
			for (__i__ = 0; __i__ < __len__; __i__++) { \
				__ast_string_field_release_active((copy)->__field_mgr_pool, *AST_VECTOR_GET(__v1__, __i__)); \
				*AST_VECTOR_GET(__v1__, __i__) = __ast_string_field_empty; \
			} \
			for (__i__ = 0; __i__ < __len__; __i__++) { \
				if (ast_string_field_ptr_set((copy), AST_VECTOR_GET(__v1__, __i__), *AST_VECTOR_GET(__v2__, __i__))) { \
					__res__ = -1; \
					break; \
				} \
			} \
