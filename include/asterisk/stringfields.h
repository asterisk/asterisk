...
({ \
	...
	if (((void *)(copy)) != NULL && ((void *)(orig)) != NULL) { \
		...
		if (__len__ == AST_VECTOR_SIZE(__v2__)) { \
			for (__i__ = 0; __i__ < __len__; __i__++) { \
				ast_string_field __c__ = *AST_VECTOR_GET(__v1__, __i__); \
				__ast_string_field_release_active((copy)->__field_mgr_pool, __c__); \
				__c__ = __ast_string_field_empty; \
			} \
			for (__i__ = 0; __i__ < __len__; __i__++) { \
				ast_string_field __c__ = *AST_VECTOR_GET(__v1__, __i__); \
				if (ast_string_field_ptr_set((copy), &__c__, *AST_VECTOR_GET(__v2__, __i__))) { \
					__res__ = -1; \
					break; \
				} \
			} \
