...
#define CODEC_TABLE_SIZE (sizeof(codec_table) / sizeof(codec_table[0]))
...
AST_TEST_DEFINE(codec_translate_roundtrip)
{
	...
	case TEST_INIT:
		...
		info->summary = "Roundtrip encode/decode test and quality check for all registered codecs";
		info->description =
	...
	ast_test_status_update(test,
		...
		(int)CODEC_TABLE_SIZE, TEST_DURATION_SECS);

	for (i = 0; i < (int)CODEC_TABLE_SIZE; i++) {
		struct ast_format *slin_fmt;
		struct ast_format *target_fmt;
		int16_t *orig_buf;
		...
		switch (rate) {
		case 16000: slin_fmt = ast_format_slin16; break;
		case 32000: slin_fmt = ast_format_slin32; break;
		case 48000: slin_fmt = ast_format_slin48; break;
		default:    slin_fmt = ast_format_slin;   break;
		}
