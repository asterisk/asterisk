...
AST_TEST_DEFINE(string_field_aggregate_test)
{
	...
	inst1 = ast_calloc_with_stringfields(4, struct test_struct, 32);
	if (!inst1) {
	...
	inst2 = ast_calloc_with_stringfields_get_ptr(inst1, 1);
	if (!inst2) {
	...
	inst3 = ast_calloc_with_stringfields_get_next_ptr(inst2);
	if (!inst3) {
	...
	inst4 = ast_calloc_with_stringfields_get_next_ptr(inst3);
	if (!inst4) {
