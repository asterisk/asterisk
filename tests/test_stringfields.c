...
AST_TEST_DEFINE(string_field_aggregate_test)
{
	...
	if (ast_string_fields_copy(inst1, inst3)) {
		ast_test_status_update(test, "Copying from structure 1 to structure 4 failed!\n");
		goto error;
	} else {
		ast_test_status_update(test, "Copying from structure 1 to structure 4 succeeded!\n");
	}
