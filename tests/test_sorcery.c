...
AST_TEST_DEFINE(wizard_apply_and_insert)
{
	...
	/* Should fail */
	ast_test_validate(test,
		ast_sorcery_remove_wizard_mapping(sorcery, "non-existent-type", 2) != 0);
	ast_test_validate(test,
		ast_sorcery_remove_wizard_mapping(sorcery, "test_object_type", 2) != 0);

	/* should work */
	ast_test_validate(test,
		ast_sorcery_remove_wizard_mapping(sorcery, "test_object_type", 1) == 0);
