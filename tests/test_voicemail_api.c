...
static struct ast_channel *test_vm_api_create_mock_channel(void)
{
	...
	ast_format_set(&mock_channel->writeformat, AST_FORMAT_GSM, 0);
	ast_format_cap_add(mock_channel->nativeformats, &mock_channel->writeformat);
	ast_format_set(&mock_channel->rawwriteformat, AST_FORMAT_GSM, 0);
	ast_format_set(&mock_channel->readformat, AST_FORMAT_GSM, 0);
	ast_format_set(&mock_channel->rawreadformat, AST_FORMAT_GSM, 0);
