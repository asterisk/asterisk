...
AST_TEST_DEFINE(basic_ops_locked)
{
	AST_RWVECTOR(test_struct, char *) sv1;
	int rc = AST_TEST_PASS;
	...
	switch (cmd) {
	case TEST_INIT:
		info->name = "basic locked";
		info->category = "/main/vector/";
		info->summary = "Test locked vector basic ops";
		info->description = "Test locked vector basic ops";
		return AST_TEST_NOT_RUN;
	case TEST_EXECUTE:
		break;
	}

	ast_test_validate(test, AST_RWVECTOR_INIT(&sv1, 3) == 0);
	test_validate_cleanup(sv1.max == 3);
	test_validate_cleanup(AST_VECTOR_SIZE(&sv1) == 0);

	test_validate_cleanup(AST_RWVECTOR_APPEND(&sv1, AAA) == 0);
	test_validate_cleanup(AST_RWVECTOR_APPEND(&sv1, BBB) == 0);
	test_validate_cleanup(AST_RWVECTOR_APPEND(&sv1, CCC) == 0);
	test_validate_cleanup(AST_RWVECTOR_SIZE(&sv1) == 3);
	test_validate_cleanup(sv1.max == 3);
	test_validate_cleanup(AST_RWVECTOR_GET(&sv1, 0) == AAA);
	test_validate_cleanup(AST_RWVECTOR_GET(&sv1, 1) == BBB);
	test_validate_cleanup(AST_RWVECTOR_GET(&sv1, 2) == CCC);

	test_validate_cleanup(AST_RWVECTOR_INSERT_AT(&sv1, 1, ZZZ) == 0);
	test_validate_cleanup(sv1.max >= 4);
	test_validate_cleanup(AST_RWVECTOR_SIZE(&sv1) == 4);
	test_validate_cleanup(AST_RWVECTOR_GET(&sv1, 0) == AAA);
	test_validate_cleanup(AST_RWVECTOR_GET(&sv1, 1) == ZZZ);
	test_validate_cleanup(AST_RWVECTOR_GET(&sv1, 2) == BBB);
	test_validate_cleanup(AST_RWVECTOR_GET(&sv1, 3) == CCC);

	test_validate_cleanup(*(char **)AST_RWVECTOR_GET_CMP(&sv1, "AAA", 0 == strcmp) == AAA);
	test_validate_cleanup(*(char **)AST_RWVECTOR_GET_CMP(&sv1, "ZZZ", 0 == strcmp) == ZZZ);

	AST_RWVECTOR_FREE(&sv1);
	ast_test_validate(test, sv1.elems == NULL);
	ast_test_validate(test, sv1.current == 0);
	ast_test_validate(test, sv1.max == 0);

	ast_test_validate(test, AST_RWVECTOR_INIT(&sv1, 0) == 0);
	test_validate_cleanup(sv1.max == 0);
	test_validate_cleanup(AST_RWVECTOR_SIZE(&sv1) == 0);
	...
	/* Overwrite index 1 */
	test_validate_cleanup(AST_RWVECTOR_REPLACE(&sv1, 1, ZZZ) == 0);
	test_validate_cleanup(AST_RWVECTOR_SIZE(&sv1) == 3);
	test_validate_cleanup(AST_RWVECTOR_GET(&sv1, 0) == AAA);
	test_validate_cleanup(AST_RWVECTOR_GET(&sv1, 1) == ZZZ);
	test_validate_cleanup(AST_RWVECTOR_GET(&sv1, 2) == CCC);
	...
	/* Remove index 0 and bring the last entry into it's empty slot */
	test_validate_cleanup(AST_RWVECTOR_REMOVE_UNORDERED(&sv1, 0) == AAA);
	test_validate_cleanup(AST_RWVECTOR_SIZE(&sv1) == 2);
	test_validate_cleanup(AST_RWVECTOR_GET(&sv1, 0) == CCC);
	test_validate_cleanup(AST_RWVECTOR_GET(&sv1, 1) == ZZZ);

	/* Replace 0 and 2 leaving 1 alone */
	test_validate_cleanup(AST_RWVECTOR_REPLACE(&sv1, 0, AAA) == 0);
	test_validate_cleanup(AST_RWVECTOR_REPLACE(&sv1, 2, CCC) == 0);
	test_validate_cleanup(AST_RWVECTOR_SIZE(&sv1) == 3);
	test_validate_cleanup(AST_RWVECTOR_GET(&sv1, 0) == AAA);
	test_validate_cleanup(AST_RWVECTOR_GET(&sv1, 1) == ZZZ);
	test_validate_cleanup(AST_RWVECTOR_GET(&sv1, 2) == CCC);
	...
	/* Remove 1 and compact preserving order */
	test_validate_cleanup(AST_RWVECTOR_REMOVE_ORDERED(&sv1, 1) == ZZZ);
	test_validate_cleanup(AST_RWVECTOR_SIZE(&sv1) == 2);
	test_validate_cleanup(AST_RWVECTOR_GET(&sv1, 0) == AAA);
	test_validate_cleanup(AST_RWVECTOR_GET(&sv1, 1) == CCC);

	/* Equivalent of APPEND */
	test_validate_cleanup(AST_RWVECTOR_REPLACE(&sv1, 2, ZZZ) == 0);
	test_validate_cleanup(AST_RWVECTOR_SIZE(&sv1) == 3);

	/* This should fail because comparison is by pointer */
	test_validate_cleanup(AST_RWVECTOR_REMOVE_ELEM_ORDERED(&sv1, "ZZZ", cleanup) != 0);
	 ...
	 */
	cleanup_count = 0;
	test_validate_cleanup(AST_RWVECTOR_REMOVE_CMP_ORDERED(&sv1, "AAA", 0 == strcmp, cleanup) == 0);
	test_validate_cleanup(AST_RWVECTOR_SIZE(&sv1) == 1);
	test_validate_cleanup(AST_RWVECTOR_GET(&sv1, 0) == CCC);
	test_validate_cleanup(cleanup_count == 1);

	/* This element is gone so we shouldn't be able to find it or delete it again. */
	test_validate_cleanup(AST_RWVECTOR_GET_CMP(&sv1, "AAA", 0 == strcmp) == NULL);
	test_validate_cleanup(AST_RWVECTOR_REMOVE_CMP_ORDERED(&sv1, "AAA", 0 == strcmp, cleanup) != 0);

	/* CCC should still be there though */
	test_validate_cleanup(*(char **)AST_RWVECTOR_GET_CMP(&sv1, "CCC", 0 == strcmp) == CCC);
...
AST_TEST_DEFINE(callbacks)
{
	...
	test_validate_cleanup(AST_VECTOR_CALLBACK(&sv2, cb, "ARG", test) == 3);

	test_validate_cleanup(AST_VECTOR_CALLBACK(&sv2, cb_first, test, test) == 1);

	test_validate_cleanup(AST_RWVECTOR_CALLBACK_RDLOCK(&sv2, cb, "ARG", test) == 3);

	test_validate_cleanup(AST_RWVECTOR_CALLBACK_RDLOCK(&sv2, cb_first, test, test) == 1);

	test_validate_cleanup(AST_RWVECTOR_CALLBACK_WRLOCK(&sv2, cb, "ARG", test) == 3);

	test_validate_cleanup(AST_RWVECTOR_CALLBACK_WRLOCK(&sv2, cb_first, test, test) == 1);
...
static int unload_module(void)
{
	...
	AST_TEST_UNREGISTER(basic_ops_locked);
...
static int load_module(void)
{
	...
	AST_TEST_REGISTER(basic_ops_locked);
