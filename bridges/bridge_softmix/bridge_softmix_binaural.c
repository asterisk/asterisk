...
#define CONVOLUTION_SAMPLE_SIZE SOFTMIX_BINAURAL_SAMPLE_RATE * BINAURAL_MIXING_INTERVAL / 1000
...
int init_convolve_channel_pair(struct convolve_channel_pair *cchan_pair, unsigned int hrtf_len, unsigned int chan_pos, unsigned int default_sample_size)
{
	...
	success = init_convolve_channel(&cchan_pair->chan_left, hrtf_len, chan_pos, HRIRS_CHANNEL_LEFT, default_sample_size);
	if (success == -1) {
		return success;
	}
	success = init_convolve_channel(&cchan_pair->chan_right, hrtf_len, chan_pos, HRIRS_CHANNEL_RIGHT, default_sample_size);
	if (success == -1) {
...
void add_binaural_mixing(struct ast_bridge *bridge, struct softmix_bridge_data *softmix_data, unsigned int softmix_samples, struct softmix_mixing_array *mixing_array, struct softmix_channel *sc, const char *channel_name)
{
	...
	if (!(bridge->softmix.binaural_active.active && softmix_data->convolve.binaural_active && softmix_samples % CONVOLUTION_SAMPLE_SIZE == 0)) {
		return;
	}
