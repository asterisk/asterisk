...
int do_convolve(struct convolve_channel *chan, int16_t *in_samples, unsigned int in_sample_size, unsigned int hrtf_length)
{
	...
	for (i = 1; i < hrtf_length / 2; i++) {
		/* Real part */
		chan->fftw_in[i] = chan->fftw_out[i] * chan->hrtf[i] - chan->fftw_out[hrtf_length - i] * chan->hrtf[hrtf_length - i];
		/* Imaginary part */
		chan->fftw_in[hrtf_length - i] = chan->fftw_out[i] * chan->hrtf[hrtf_length - i] + chan->fftw_out[hrtf_length - i] * chan->hrtf[i];
	}
...
{
#ifdef HAVE_FFTW3
	if (chan_side == IMPULSE_CHANNEL_LEFT) {
		return hrirs_left[ast_binaural_positions[chan_pos]];
	} else if (chan_side == IMPULSE_CHANNEL_RIGHT) {
		return hrirs_right[ast_binaural_positions[chan_pos]];
	}
#else
	ast_log(LOG_ERROR, "Requesting data for the binaural conference feature without it beeing active.\n");
#endif
	return NULL;
}
...
int init_convolve_channel_pair(struct convolve_channel_pair *cchan_pair, unsigned int hrtf_len, unsigned int chan_pos, unsigned int default_sample_size)
{
	...
	success = init_convolve_channel(&cchan_pair->chan_left, hrtf_len, chan_pos, IMPULSE_CHANNEL_LEFT, default_sample_size);
	if (success == -1) {
		return success;
	}
	success = init_convolve_channel(&cchan_pair->chan_right, hrtf_len, chan_pos, IMPULSE_CHANNEL_RIGHT, default_sample_size);
	if (success == -1) {
...
int init_convolve_data(struct convolve_data *data, unsigned int default_sample_size)
{
	...
	for (i = 0; i < CONVOLVE_CHANNEL_PREALLOC; i++) {
		success = init_convolve_channel_pair(data->cchan_pair[i], data->hrtf_length, i, default_sample_size);
		if (success == -1) {
			free(data->pos_ids);
			for (j = 0; j < i; j++) {
...
int set_binaural_data_join(struct convolve_data *data, unsigned int default_sample_size)
{
	...
	if (data->chan_size < data->number_channels)  {
		data->chan_size += 1;
		pos_ids_tmp = realloc(data->pos_ids, data->chan_size * sizeof(int));
		if (pos_ids_tmp) {
		...
		cchan_pair_tmp = realloc(data->cchan_pair, data->chan_size * sizeof(struct convolve_channel_pair *));
		if (cchan_pair_tmp) {
		...
		i = init_convolve_channel_pair(data->cchan_pair[data->chan_size - 1], data->hrtf_length, data->chan_size - 1, default_sample_size);
		if (i == -1) {
			goto binaural_join_fails;
		}
	}
...
void softmix_process_write_binaural_audio(struct softmix_channel *sc, unsigned int default_sample_size)
{
  ...
  /* If binaural is NOT suspended, the source audio (binaural) will be removed. */
	for (i = 0; i < default_sample_size; i++) {
		ast_slinear_saturated_subtract(&sc->final_buf[i * 2], &sc->our_chan_pair->chan_left.out_data[i]);
		ast_slinear_saturated_subtract(&sc->final_buf[i * 2 + 1], &sc->our_chan_pair->chan_right.out_data[i]);
	}
}
...
void check_binaural_position_change(struct ast_bridge *bridge, struct softmix_bridge_data *softmix_data, struct ast_bridge_channel *bridge_channel)
{
	...
	/* Before we pull any audio, we must check if any channel requests a change of binaural positions. */
	pos_change = 0;
...
void add_binaural_mixing(struct ast_bridge *bridge, struct softmix_bridge_data *softmix_data, unsigned int softmix_samples, struct softmix_mixing_array *mixing_array, struct softmix_channel *sc, const char *channel_name)
{
	...
	if (!(bridge->softmix.binaural_active.active && softmix_data->convolve.binaural_active && softmix_samples % DEFAULT_SAMPLE_SIZE == 0)) {
		return;
	}
	if (!sc->is_announcement) {
		pair = do_convolve_pair(&softmix_data->convolve, sc->binaural_pos, mixing_array->buffers[mixing_array->used_entries], softmix_samples, channel_name);
	}
...
void binaural_mixing(struct ast_bridge *bridge, struct softmix_bridge_data *softmix_data, struct softmix_mixing_array *mixing_array, int16_t *bin_buf, int16_t *ann_buf)
{
	...
	for (idx = 0; idx < mixing_array->used_entries; idx++) {
		if (mixing_array->chan_pairs[idx] == NULL) {
			...
			for (x = 0; x < softmix_data->default_sample_size; x++) {
				ast_slinear_saturated_add(bin_buf + x * 2, mixing_array->chan_pairs[idx]->chan_left.out_data + x);
				ast_slinear_saturated_add(bin_buf + x * 2 + 1, mixing_array->chan_pairs[idx]->chan_right.out_data + x);
			}
		}
	}
}
...
void create_binaural_frame(struct ast_bridge_channel *bridge_channel, struct softmix_channel *sc, int16_t *bin_buf, int16_t *ann_buf, unsigned int softmix_datalen, unsigned int softmix_samples, int16_t *buf)
{
	...
	/* Mark that binaural output is suspended, since we use two channel audio we copy the same signals into both channels. */
