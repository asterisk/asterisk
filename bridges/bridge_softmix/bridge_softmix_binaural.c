...
int do_convolve(struct convolve_channel *chan, int16_t *in_samples, int in_sample_size, int hrtf_length)
{
#ifdef HAVE_FFTW3
	int i;
...
struct convolve_channel_pair *do_convolve_pair(struct convolve_data *data, unsigned int pos_id, int16_t *in_samples, int in_sample_size, const char *channel_name)
{
	int status;
...
float *get_hrir(int chan_pos, int chan_side)
{
#ifdef HAVE_FFTW3
...
int init_convolve_channel(struct convolve_channel *channel, int hrtf_len, unsigned int chan_pos, unsigned int chan_side, unsigned int default_sample_size)
{
#ifdef HAVE_FFTW3
	int j;
	float *hrir;
...
int init_convolve_channel_pair(struct convolve_channel_pair *cchan_pair, int hrtf_len, unsigned int chan_pos, unsigned int default_sample_size)
{
#ifdef HAVE_FFTW3
	...
	return success;
#else
	ast_log(LOG_ERROR, "Requesting data for the binaural conference future without it beeing active.\n");
	return -1;
#endif
}
