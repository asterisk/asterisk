...
/*!
 * \internal
 * \brief Helper function for rtp_glue_data_get
 *
	...
	struct ast_channel *c1, struct rtp_glue_data *glue1)
{
	...
	ast_debug(2, "%p, From media bridging, glue audio result 1: %d, 2: %d\n", glue0->cb->allow_rtp_remote, glue0->audio.result, glue1->audio.result);
	...
	ast_debug(2, "%p, From media bridging, glue video result 1: %d, 2: %d\n", glue0->cb->allow_vrtp_remote, glue0->video.result, glue1->video.result);
	...
	ast_debug(2, "%p, From media bridging, glue text result 1: %d, 2: %d\n", glue0->cb->allow_trtp_remote, glue0->text.result, glue1->text.result);
	 ...
	 * The order of preferences for media types its audio, video and text.
	 */
