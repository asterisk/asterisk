...
{
	if (!ast_strings_equal(
		ast_format_get_name(ast_channel_rawreadformat(instance->channel)),
...
static struct ast_frame *dequeue_frame(struct websocket_pvt *instance)
{
	...
	native_frame = AST_LIST_REMOVE_HEAD(&instance->frame_queue, frame_list);
	/*
	 * If there are no frames in the queue, we need to
	 * return NULL so we can send a silenmce frame.
	 */
	if (!native_frame) {
		return NULL;
	}
	 ...
	 * frame anyway.
	 *
	 * It's possible although unlikely that there are multiple
	 * control frames in a row in the queue so we need to
	 * process consecutive ones immediately.
	 *
	 ...
	 */
	while (native_frame && native_frame->frametype == AST_FRAME_CONTROL) {
		if (native_frame->subclass.integer == AST_CONTROL_OPTION) {
			/*
			...
			ast_websocket_write_string(instance->websocket,
				native_frame->data.ptr);
			ast_debug(4, "%s: WebSocket sending %s\n",
				ast_channel_name(instance->channel), (char *)native_frame->data.ptr);
		}
		/*
		...
		native_frame = AST_LIST_REMOVE_HEAD(&instance->frame_queue, frame_list);
		/*
		 * Jut FYI... We don't bump the queue length when we add control
	...
	} else if (instance->report_queue_drained && instance->frame_queue_length == 0) {
...
static struct ast_frame *webchan_read(struct ast_channel *ast)
{
	...
	if (instance->translator) {
		/*
		 * ast_translate will free native_frame.
		 */
...
static void *read_thread_handler(void *obj)
{
	 ...
	 */
	res = ast_asprintf(&command, "%s connection_id:%s channel:%s optimal_frame_size:%d", MEDIA_START,
