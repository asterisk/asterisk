...
/*
 * \brief Try to acquire the iaxsl[callno] if call exists and not having ongoing hangup.
 ...
 *
 * Try to acquire the iaxsl[callno] MUTEX lock to avoid the deadlock that might
 * happen, in case a hangup thread locks the MUTEX before this thread does and
 * then tries to lock it one more time while this thread is already locked and
 * waiting for the MUTEX to be released. See ASTERISK-24983.
 */
static int iax_try_lock_unless_hungup(int callno){
	while (ast_mutex_trylock(&iaxsl[callno]) != 0){
		/*Return if callno disappeared OR there is ongoing hang up procedure.*/
		if(!iaxs[callno] || (iaxs[callno] && iaxs[callno]->destroy_initiated)) {
			ast_debug(3, "I wanted to lock callno %d, but it is dead or going to die.\n", callno);
			return 0;
		}
		ast_debug(3,"Waiting for the callno %d hangup to finish\n", callno);
		usleep(100);
	}
	return 1;
}
...
static void __send_ping(const void *data)
{
	...
	if (iax_try_lock_unless_hungup(callno) == 0) {
...
static void iax2_destroy_helper(struct chan_iax2_pvt *pvt)
{
	...
	/* Remove scheduled (but didn't run yet) PINGs or LAGRQs */
