...
static struct regen_sdp_data *regen_sdp_data_alloc(struct ast_sip_session *session, int is_offer);
...
static int chan_pjsip_answer_with_stream_topology(struct ast_channel *ast, struct ast_stream_topology *topology)
{
	...
	if (topology != NULL) {
		...
		if (resolved_top && channel->session->pending_media_state->topology) {
			struct regen_sdp_data *regen_data;
			int is_offer;

			ast_channel_set_stream_topology(ast, resolved_top);
			ast_stream_topology_free(channel->session->pending_media_state->topology);
			channel->session->pending_media_state->topology = ast_stream_topology_clone(resolved_top);

			is_offer = pjmedia_sdp_neg_get_state(channel->session->inv_session->neg) == PJMEDIA_SDP_NEG_STATE_LOCAL_OFFER;
			regen_data = regen_sdp_data_alloc(channel->session, is_offer);
			if (!regen_data) {
			...
			ast_log(LOG_NOTICE, "No resolved topology or pending_media_state topology. Still answering call.\n");
		}
	}
