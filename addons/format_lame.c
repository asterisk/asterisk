...
static int mp3lame_hip_decode(hip_global_flags * handle, int input_size, unsigned char * input, short int * output, int sample_rate)
{
	...
	if (headers.header_parsed == 1) {
		ast_debug(3, "LAME decoder found MP3 headers: channels=%d, samplerate=%d, bitrate=%d, framesize=%d, mode=[%d:%d]",
			headers.stereo,
			headers.samplerate,
			headers.bitrate,
			headers.framesize,
...
static int mp3lame_file_read(struct ast_filestream *s, int sample_rate)
{
	...
	ast_debug(6, "LAME decoder input: %d bytes", rres);
	...
	ast_debug(6, "LAME decoder got %d output samples", p->dsamples);
...
static int load_module(void)
{
	...
	ast_debug(1, "LAME version: %s", get_lame_version());
