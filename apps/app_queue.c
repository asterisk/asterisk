...
static int qupd_exec(struct ast_channel *chan, const char *data)
{
	...
	if (!q) {
		ast_log(LOG_WARNING, "Unable to update member '%s' in queue '%s' as the queue does not exist\n", args.membername, args.queuename);
		return 0;
	}

	ao2_lock(q);
	if (q->members) {
		mem = ao2_find(q->members, args.membername, OBJ_KEY);
	}
	ao2_unlock(q);
	...
	if (mem) {
		...
		ast_log(LOG_WARNING, "Unable to update member '%s' in queue '%s' as the member does not exist\n", args.membername, args.queuename);
	}
