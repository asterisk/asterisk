...
/*! \file
 *
 * \brief SendFax application for res_fax
 *
 * \author\verbatim Dwayne M. Hubbard <dhubbard@digium.com> \endverbatim
 * 
 * This is a SendFax application for use with res_fax.  This is probably
...
static char *app = "SendFax";
static char *synopsis = 
"SendFax application for res_fax.";
static char *descrip = "This application is a ?temporary? SendFax application.\n"
...
static int app_exec(struct ast_channel *chan, void *data)
{
	...
	sessionreqs->operation = FAX_SESSION_OP_SEND;
	...
	while (ms > 0) {
		...
		if (frame->frametype == AST_FRAME_CONTROL && frame->subclass == AST_CONTROL_T38 && frame->datalen == sizeof(enum ast_control_t38)) {
			...
			if (t38control == AST_T38_NEGOTIATED) {
				ast_log(LOG_NOTICE, "yay T38 negotiated!\n");
				session->start(session);
			} else ast_log(LOG_NOTICE, "T38 control frame: %d\n", t38control);
		}
		ast_frfree(frame);
	}
...
AST_MODULE_INFO_STANDARD(ASTERISK_GPL_KEY, "SendFax Application for res_fax");
