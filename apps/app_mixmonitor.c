			...
			<parameter name="options">
				<optionlist>
					...
					<option name="s">
						...
						<para>Skip the first <replaceable>seconds</replaceable> of audio in the recording.
	...
	if (args.options) {
		...
		if (ast_test_flag(&flags, MUXFLAG_SKIP)) {
			if (ast_strlen_zero(opts[OPT_ARG_SKIP])) {
				...
				ast_log(LOG_WARNING, "The skip will be ignored, the value must be a number >= %d, not '%s'\n", MIN_SKIP_SECONDS, opts[OPT_ARG_SKIP]);
			} else {
				skip_seconds = x;
			}
		}
