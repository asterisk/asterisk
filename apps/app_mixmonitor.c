			...
			<parameter name="options">
				<optionlist>
					...
					<option name="s">
						...
						<para>Skip the first <replaceable>seconds</replaceable> (1–120) of audio in the recording.
...
struct mixmonitor {
	...
	/* Number of seconds to skip at the start of recording */
	int skip_seconds;
...
static void *mixmonitor_thread(void *obj)
{
	...
	skip_samples = mixmonitor->skip_seconds * mixmonitor->mixmonitor_ds->samp_rate;
	skipped_samples = 0;
	...
	while (mixmonitor->audiohook.status == AST_AUDIOHOOK_STATUS_RUNNING && !mixmonitor->mixmonitor_ds->fs_quit) {
		...
		if (!ast_test_flag(mixmonitor, MUXFLAG_BRIDGED)
			...
			/* Skip writing frames until skip_samples is reached */
			frame_samples = fr ? fr->samples : 0;
...
static int launch_monitor_thread(struct ast_channel *chan, const char *filename,
				  ...
				  const char *recipients, const char *beep_id, int skip_seconds)
{
	pthread_t thread;
...
{
	int x, readvol = 0, writevol = 0, skip_seconds = 0;
	...
	if (args.options) {
		...
		if (ast_test_flag(&flags, MUXFLAG_SKIP)) {
			if (ast_strlen_zero(opts[OPT_ARG_SKIP])) {
				ast_log(LOG_WARNING, "No skip value was provided for the skip ('s') option.\n");
			} else if ((sscanf(opts[OPT_ARG_SKIP], "%3d", &x) != 1) || (x < MIN_SKIP_SECONDS)) {
				ast_log(LOG_NOTICE, "The skip will be ignored, the value must be a number between %d and %d, not '%s'\n", MIN_SKIP_SECONDS, MAX_SKIP_SECONDS, opts[OPT_ARG_SKIP]);
			} else if (x > MAX_SKIP_SECONDS) {
				ast_log(LOG_NOTICE, "The skip value %d is too high, clamping to %d seconds.\n", x, MAX_SKIP_SECONDS);
				skip_seconds = MAX_SKIP_SECONDS;
			} else {
				skip_seconds = x;
			}
		}
