...
static int send_unpublish_task(void *data)
{
	...
	if (pjsip_publishc_unpublish(publisher->client, &tdata) == PJ_SUCCESS) {
		if (!ast_strlen_zero(publisher->transport_name)) {
			pjsip_tpselector selector = { .type = PJSIP_TPSELECTOR_NONE, };
			ast_sip_set_tpselector_from_transport_name(publisher->transport_name, &selector);
...
static void sip_outbound_publish_synchronize(struct ast_sip_event_publisher_handler *removed)
{
	...
	while ((state = ao2_iterator_next(&i))) {
		...
		if (!state->client->started) {
			...
			ao2_callback(state->client->publishers, OBJ_NODATA, cancel_and_unpublish, removed);
		}
		ao2_ref(publish, -1);
		ao2_ref(state, -1);
	}
...
static void sip_outbound_publisher_destroy(void *obj)
{
	...
	ast_free(publisher->transport_name);
	ast_free(publisher->user);
	...
	struct ast_sip_outbound_publish_client *client, const char *user)
{
	...
	publisher = ao2_alloc(sizeof(*publisher), sip_outbound_publisher_destroy);
	if (!publisher) {
		return NULL;
	}

	/*
	 * Bump the ref in order to make sure the client object doesn't get
	 * pulled out from under us when we stop publishing.
	 */
	publisher->owner = ao2_bump(client);
	publisher->timer.user_data = publisher;
	publisher->timer.cb = sip_outbound_publish_timer_cb;
	publisher->transport_name = ast_strdup(client->publish->transport);
	publisher->user = ast_strdup(user ? user : "");
...
{
	struct sip_outbound_publisher *publisher =
		ao2_find(client->publishers, user, OBJ_SEARCH_KEY);
	int res;
...
static int cancel_and_unpublish(void *obj, void *arg, int flags)
{
	...
	struct ast_sip_event_publisher_handler *handler = arg;
	SCOPED_AO2LOCK(lock, publisher);
...
static void sip_outbound_publish_state_destroy(void *obj)
{
	...
	/* Need to clean up the publishers here so things actually stop */
