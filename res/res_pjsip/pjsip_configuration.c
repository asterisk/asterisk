...
static int sip_endpoint_apply_handler(const struct ast_sorcery *sorcery, void *obj)
{
	...
	if (endpoint->media.rtp.port_start || endpoint->media.rtp.port_end) {
		...
		if (endpoint->media.rtp.port_end > 65535) {
			ast_log(LOG_ERROR, "Endpoint '%s': rtp_port_end must not exceed 65535\n",
				ast_sorcery_object_get_id(endpoint));
			return -1;
		}
		...
		ast_log(LOG_NOTICE, "Endpoint '%s': Using per-endpoint RTP port range %u-%u\n",
			ast_sorcery_object_get_id(endpoint),
			endpoint->media.rtp.port_start,
			endpoint->media.rtp.port_end);
	}
...
int ast_res_pjsip_initialize_configuration(void)
{
	...
	ast_sorcery_object_field_register(sip_sorcery, "endpoint", "rtp_port_start", "0", OPT_UINT_T, 0, FLDSET(struct ast_sip_endpoint, media.rtp.port_start));
	ast_sorcery_object_field_register(sip_sorcery, "endpoint", "rtp_port_end", "0", OPT_UINT_T, 0, FLDSET(struct ast_sip_endpoint, media.rtp.port_end));
