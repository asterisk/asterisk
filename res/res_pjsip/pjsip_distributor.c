...
static char *cli_show_unidentified_requests(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a)
{
	...
	ao2_ref(unidentified_requests, +1);
	ao2_rdlock(unidentified_requests);
	i = ao2_iterator_init(unidentified_requests, 0);
...
static int prune_task(const void *data)
{
	...
	return unidentified_prune_interval;
}
...
static void global_loaded(const char *object_type)
{
	...
	if (ast_sched_add_variable(prune_context, unidentified_prune_interval, prune_task, NULL, 1) < 0) {
		return;
	}
}
