...
static struct internal_state *find_internal_state_by_transport(struct ast_sip_transport *transport, enum state_types type)
{
	const char *key;
...
static struct ast_sip_transport_state *find_state_by_transport(const struct ast_sip_transport *transport, enum state_types type)
{
...
static struct internal_state *internal_state_alloc(struct ast_sip_transport *transport, enum state_types type)
{
	...
	if (type == PERMANENT_STATE) {
		...
		internal_state->id = ast_malloc(MAX_POINTER_STRING);
		if (!internal_state->id) {
			ao2_cleanup(internal_state);
			return NULL;
		}
		snprintf(internal_state->id, MAX_POINTER_STRING, "%p", transport);
	}
...
static struct ast_sip_transport_state *find_or_create_temporary_state(struct ast_sip_transport *transport)
{
	...
	state = find_state_by_transport(transport, TEMPORARY_STATE);
	if (state) {
		return state;
	}

	internal_state = internal_state_alloc(transport, TEMPORARY_STATE);
	if (!internal_state) {
		return NULL;
	}

	ao2_link_flags(transport_states, internal_state, OBJ_NOLOCK);
	/* Transfer the ref to the container */
	ao2_ref(internal_state, -1);

	return ao2_bump(internal_state->state);
}
...
static int transport_apply(const struct ast_sorcery *sorcery, void *obj)
{
	...
	temp_state = find_internal_state_by_transport(transport, TEMPORARY_STATE);
	/* Something is seriously wrong if we don't have temp_state */
	ast_assert(temp_state != NULL);
...
{
	const struct ast_sip_transport *transport = obj;
	RAII_VAR(struct ast_sip_transport_state *, state, find_state_by_transport(transport, PERMANENT_STATE), ao2_cleanup);
