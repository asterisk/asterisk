...
static int transport_apply(const struct ast_sorcery *sorcery, void *obj)
{
	...
	RAII_VAR(struct ao2_container *, states, ao2_global_obj_ref(current_states), states_cleanup);
	...
	if (old_state) {
		/* We want the old values in changes so we can apply them back to the new transport */
		ast_sorcery_diff(sorcery, transport, old_state->transport, &changes);
		if (changes) {
...
int ast_sip_initialize_sorcery_transport(void)
{
	...
	new_states = ao2_container_alloc(DEFAULT_STATE_BUCKETS, internal_state_hash, internal_state_cmp);
	if (!new_states) {
