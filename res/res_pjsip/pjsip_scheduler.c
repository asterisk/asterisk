...
int ast_sip_sched_task_get_next_run(struct ast_sip_sched_task *schtd)
{
	...
	if (schtd->interval) {
		delay = schtd->interval;
		if (schtd->flags & AST_SIP_SCHED_TASK_DELAY) {
			since_when = schtd->is_running ? ast_tvnow() : schtd->last_end;
		} else {
...
static void schtd_destructor(void *data)
{
	...
	if (ao2_is_object_valid(schtd->task_data)) {
		/* release our own ref, then release the callers if asked to do so */
		ao2_ref(schtd->task_data, (schtd->flags & AST_SIP_SCHED_TASK_DATA_UNREF) ? -2 : -1);
...
#define ID_LEN 13 /* task_deadbeef */
	...
	if (ao2_is_object_valid(task_data)) {
		ao2_ref(task_data, +1);
	}
