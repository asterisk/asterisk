...
int ast_sip_should_redirect(struct ast_sip_endpoint *endpoint, int status_code)
{
	...
	if (endpoint->redirect_method != AST_SIP_REDIRECT_URI_PJSIP) {
		ast_log(LOG_NOTICE, "Received %d redirect while sending message to %s, but redirect_method is not 'uri_pjsip' (it is '%s'). Not following redirect.\n",
			status_code,
...
int ast_sip_redirect_parse_3xx(pjsip_rx_data *rdata, struct ast_sip_redirect_state *state)
{
	...
	/* Check redirect_method on endpoint */
	if (!ast_sip_should_redirect(state->endpoint, status_code)) {
		return -1;
	}
