...
static void ctx_destructor(void *obj)
{
	...
	ao2_cleanup(ctx->as_cfg);
	ao2_cleanup(ctx->profile);
	ao2_cleanup(ctx->tn);
		...
		const char *tag, struct ast_stir_shaken_as_ctx **ctxout)
{
	...
	RAII_VAR(struct ss_as_cfg *, as_cfg, NULL, ao2_cleanup);
	RAII_VAR(struct ss_tn *, tn, NULL, ao2_cleanup);
	...
	tn = ss_tn_get(orig_tn);
	if (!tn) {
		SCOPE_EXIT_RTN_VALUE(AST_STIR_SHAKEN_AS_DISABLED,
			"%s: No tn for orig_tn '%s'\n", tag, orig_tn);
	}

	as_cfg = ss_get_as_cfg();
	...
	if (profile->behavior != AST_STIR_SHAKEN_BEHAVIOR_ON &&
		profile->behavior != AST_STIR_SHAKEN_BEHAVIOR_ATTEST) {
		SCOPE_EXIT_RTN_VALUE(AST_STIR_SHAKEN_AS_DISABLED,
	...
	ctx->attest_level = EFFECTIVE_ENUM(tn, profile, attest_level,
		AST_STIR_SHAKEN_ATTEST_LEVEL, as_cfg->attest_level);
	if (ctx->attest_level == AST_STIR_SHAKEN_ATTEST_LEVEL_NOT_SET) {
	...
	ctx->send_mky = EFFECTIVE_ENUM_BOOL(profile, as_cfg, send_mky,
		AST_STIR_SHAKEN_AS_SEND_MKY, AST_STIR_SHAKEN_AS_SEND_MKY_NO);

	if (!ast_strlen_zero(tn->public_cert_url)) {
		ctx->public_cert_url = tn->public_cert_url;
		ctx->tn = ao2_bump(tn);
	} else if (!ast_strlen_zero(profile->public_cert_url)) {
		ctx->public_cert_url = profile->public_cert_url;
		ctx->profile = ao2_bump(profile);
	} else if (!ast_strlen_zero(as_cfg->public_cert_url)) {
		ctx->public_cert_url = as_cfg->public_cert_url;
		ctx->as_cfg = ao2_bump(as_cfg);
	} else {
		SCOPE_EXIT_LOG_RTN_VALUE(AST_STIR_SHAKEN_AS_NO_PUBLIC_CERT_URL_AVAIL,
			LOG_ERROR, "%s: No public cert url in tn, profile or attestation objects\n", tag);
	}

	if (tn->raw_key_length) {
		ctx->raw_key_length = tn->raw_key_length;
		ctx->raw_key = tn->raw_key;
		if (!ctx->tn) {
			ctx->tn = ao2_bump(tn);
		}
	} else if (profile->raw_key_length) {
		ctx->raw_key_length = profile->raw_key_length;
		ctx->raw_key = profile->raw_key;
		if (!ctx->profile) {
			ctx->profile = ao2_bump(profile);
		}
	} else if (as_cfg->raw_key_length) {
		ctx->raw_key_length = as_cfg->raw_key_length;
		ctx->raw_key = as_cfg->raw_key;
		if (!ctx->as_cfg) {
			ctx->as_cfg = ao2_bump(as_cfg);
		}
	} else {
		SCOPE_EXIT_LOG_RTN_VALUE(AST_STIR_SHAKEN_AS_NO_PRIVATE_KEY_AVAIL,
			LOG_ERROR, "%s: private key in tn, profile or attestation objects\n", tag);
	}
	...
	struct ast_stir_shaken_as_ctx *ctx, jwt_t *jwt)
{
	 ...
	 * to it parent as soon as it's created, then add things
	...
	CREATE_JSON_SET_OBJ(ast_json_string_create(
		ast_stir_shaken_attest_level_to_str(ctx->attest_level)),
		payload, "attest");
	...
	if (AST_VECTOR_SIZE(&ctx->fingerprints) && ctx->send_mky) {
		struct ast_json *mky;
		int i;
	...
	struct ast_stir_shaken_as_ctx *ctx, char **header)
{
	...
	SCOPE_ENTER(3, "%s: Attestation: orig: %s dest: %s\n",
		ctx->tag, ctx->orig_tn, ctx->dest_tn);
	...
	jwt_set_alg(jwt, alg, (const unsigned char *)ctx->raw_key,
		ctx->raw_key_length);
	jwt_add_header(jwt, "ppt", STIR_SHAKEN_PPT);
	jwt_add_header(jwt, "typ", STIR_SHAKEN_TYPE);
	jwt_add_header(jwt, "x5u", ctx->public_cert_url);
	...
	rc = ast_asprintf(header, "%s;info=<%s>;alg=%s;ppt=%s",
		encoded, ctx->public_cert_url, jwt_alg_str(alg), STIR_SHAKEN_PPT);
...
int ss_as_load()
{
	if (ss_as_config_load()) {
		return AST_MODULE_LOAD_DECLINE;
	}

	if (!ss_as_is_config_loaded()) {
		ast_log(LOG_ERROR, "stir/shaken attestation service failed to load\n");
		return AST_MODULE_LOAD_DECLINE;
	}
