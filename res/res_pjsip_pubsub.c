...
static int isEvsubDestroyed(pjsip_evsub *evsub)
{
...
static int serialized_send_notify(void *userdata)
{
	...
	if (isEvsubDestroyed(sub_tree->evsub) || sub_tree->pending_terminates) {
		...
		int terminate)
{
	...
	if (!sub->tree->evsub || isEvsubDestroyed(sub->tree->evsub) || sub->tree->pending_terminates) {
		pjsip_dlg_dec_lock(dlg);
		return 0;
	}
...
static int serialized_pubsub_on_server_timeout(void *userdata)
{
	...
	pjsip_dlg_inc_lock(dlg);
	if (!sub_tree->evsub || isEvsubDestroyed(sub_tree->evsub)) {
		pjsip_dlg_dec_lock(dlg);
		ao2_cleanup(sub_tree);
		return 0;
	}
...
static int serialized_pubsub_on_rx_refresh(void *userdata)
{
	...
	pjsip_dlg_inc_lock(dlg);
	if (!sub_tree->evsub || isEvsubDestroyed(sub_tree->evsub) || sub_tree->pending_terminates) {
		pjsip_dlg_dec_lock(dlg);
		ao2_cleanup(sub_tree);
		return 0;
	}
		...
		int *p_st_code, pj_str_t **p_st_text, pjsip_hdr *res_hdr, pjsip_msg_body **p_body)
{
	...
	if ((!(sub_tree = pjsip_evsub_get_mod_data(evsub, pubsub_module.id)))
		 || isEvsubDestroyed(sub_tree->evsub)) {
		return;
	}
...
static void pubsub_on_server_timeout(pjsip_evsub *evsub)
{
	...
	if (!(sub_tree = pjsip_evsub_get_mod_data(evsub, pubsub_module.id))
		 || isEvsubDestroyed(sub_tree->evsub) || sub_tree->pending_terminates) {
