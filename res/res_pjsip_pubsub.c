...
static int sub_persistence_recreate(void *obj)
{
	...
	if (PJSIP_IS_STATUS_IN_CLASS(resp, 200)) {
		...
		if (!sub_tree) {
			...
			if (!ind) {
				ast_log(LOG_ERROR, "Unable to allocate memory for initial_notify_data\n");
...
static void pubsub_on_evsub_state(pjsip_evsub *evsub, pjsip_event *event)
{
	...
	ast_debug(3, "on_evsub_state called on %p with state %s event %s\n", evsub,
		pjsip_evsub_get_state_name(evsub), pjsip_event_str(event->type));
