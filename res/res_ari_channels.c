 ...
 * \param[out] response Response to the HTTP request.
 */
static void ast_ari_channels_send_json_cb(
	struct ast_tcptls_session_instance *ser,
	struct ast_variable *get_params, struct ast_variable *path_vars,
	struct ast_variable *headers, struct ast_json *body, struct ast_ari_response *response)
{
	...
	args.data = body;
	// TODO: body cannot be and empty json.
	...
	default:
		if (200 <= code && code <= 299) {
			...
			ast_log(LOG_ERROR, "Invalid error response %d for /channels/{channelId}/json\n", code);
			is_valid = 0;
		}
	}

	if (!is_valid) {
		ast_log(LOG_ERROR, "Response validation failed for /channels/{channelId}/json\n");
		ast_ari_response_error(response, 500,
			"Internal Server Error", "Response validation failed");
	}
#endif /* AST_DEVMODE */
...
/*! \brief REST handler for /api-docs/channels.json */
static struct stasis_rest_handlers channels_channelId_json = {
	.path_segment = "json",
	.callbacks = {
		[AST_HTTP_POST] = ast_ari_channels_send_json_cb,
	},
	.num_children = 0,
	.children = {  }
};
