...
static int mwi_serializer_set_alert_levels(void)
{
	...
	if (!mwi_serializer_pool[0]) {
		ast_log(AST_LOG_WARNING, "The MWI serializer pool is not inizialazed. Can not set alert levels\n");
		return -1;
	}

	tps_queue_high = (long) ast_sip_get_mwi_tps_queue_high();
	if (tps_queue_high <= 0) {
		ast_log(AST_LOG_WARNING, "Invalid the taskprocessor high water alert trigger level '%ld'\n", tps_queue_high);
		tps_queue_high = AST_TASKPROCESSOR_HIGH_WATER_LEVEL;
	}

	tps_queue_low = (long) ast_sip_get_mwi_tps_queue_low();
	if (tps_queue_low <= 0 || tps_queue_low >= tps_queue_high) {
		ast_log(AST_LOG_WARNING, "Invalid the taskprocessor low water clear alert level '%ld'\n", tps_queue_low);
		tps_queue_low = -1;
	}
