...
int AST_OPTIONAL_API_NAME(ast_websocket_write)(struct ast_websocket *session, enum ast_websocket_opcode opcode, char *payload, uint64_t payload_size)
{
	...
	if (ast_iostream_write(session->stream, frame, frame_size)) {
		ao2_unlock(session);
...
int AST_OPTIONAL_API_NAME(ast_websocket_uri_cb)(struct ast_tcptls_session_instance *ser, const struct ast_http_uri *urih, const char *uri, enum ast_http_method method, struct ast_variable *get_vars, struct ast_variable *headers)
{
	...
	if (version == 7 || version == 8 || version == 13) {
		...
		ast_iostream_write(session->stream, buf, len);
	} else {
	...
	struct websocket_client *client)
{
	...
	if (ast_iostream_write(client->ser->stream, buf, len) != len) {
...
static enum ast_websocket_result websocket_client_connect(struct ast_websocket *ws)
{
	...
	ws->secure = ast_iostream_get_ssl(ws->client->ser->stream) ? 1 : 0;
