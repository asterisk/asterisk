...
AST_THREADSTORAGE(transport_management_thread_storage);
...
static int idle_sched_cb(const void *data)
{
	...
	if (!pj_thread_is_registered()) {
		...
		desc = ast_threadstorage_get(&transport_management_thread_storage,
				sizeof(pj_thread_desc));
		if (!desc) {
			ast_log(LOG_ERROR, "Could not get thread desc from thread-local storage."
					" Unable to shut down idle connections.\n");
			return 0;
		}
