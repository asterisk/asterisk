...
{
	struct exten_state_publisher *publisher;
	size_t name_size, body_type_size, body_subtype_size;
	char *body_subtype;
	const char *body_type, *context, *exten;

	name_size = strlen(ast_sorcery_object_get_id(configuration)) + 1;

	if (ast_strlen_zero(ast_sorcery_object_get_extended(configuration, "body"))) {
		ast_log(LOG_ERROR, "Outbound extension state publisher '%s' has no body set\n",
			ast_sorcery_object_get_id(configuration));
		return -1;
	}

	body_subtype = ast_strdupa(ast_sorcery_object_get_extended(configuration, "body"));

	body_type = strsep(&body_subtype, "/");
	if (ast_strlen_zero(body_type)) {
		ast_log(LOG_ERROR, "Outbound extension state publisher '%s' has no body type set\n",
			ast_sorcery_object_get_id(configuration));
		return -1;
	}
	body_type_size = strlen(body_type) + 1;

	if (ast_strlen_zero(body_subtype)) {
		ast_log(LOG_ERROR, "Outbound extension state publisher '%s' has no body subtype set\n",
			ast_sorcery_object_get_id(configuration));
		return -1;
	}
	body_subtype_size = strlen(body_subtype) + 1;

	publisher = ao2_alloc_options(sizeof(*publisher) + name_size + body_type_size + body_subtype_size,
	...
	publisher->body_subtype = publisher->name + name_size + body_type_size;
	...
	if (!ast_strlen_zero(context)) {
		if (build_regex(&publisher->context_regex, context)) {
			ast_log(LOG_ERROR, "Could not build context filter '%s' on publisher %s\n",
				context, ast_sorcery_object_get_id(configuration));
			ao2_ref(publisher, -1);
			return -1;
		}
	...
	if (!ast_strlen_zero(exten)) {
		if (build_regex(&publisher->exten_regex, exten)) {
			ast_log(LOG_ERROR, "Could not build exten filter '%s' on publisher %s\n",
				exten, ast_sorcery_object_get_id(configuration));
			ao2_ref(publisher, -1);
			return -1;
		}
