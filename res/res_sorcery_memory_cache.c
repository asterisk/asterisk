...
static void *sorcery_memory_cache_open(const char *data)
{
	...
	while (!ast_strlen_zero(options) && (option = strsep(&options, ","))) {
		...
		if (!strcasecmp(name, "name")) {
			...
			if (ast_strlen_zero(value)) {
				ast_log(LOG_ERROR, "A value for object maximum lifetime must be provided for memory cache\n");
				return NULL;
			} else if (!strncmp(value, "-", 1)) {
				ast_log(LOG_ERROR, "An invalid negative value '%s' has been provided for object maximum lifetime for memory cache\n",
					value);
				return NULL;
			} else if (sscanf(value, "%30u", &cache->object_lifetime_maximum) != 1) {
			...
			if (ast_strlen_zero(value)) {
				ast_log(LOG_ERROR, "A value for object stale lifetime must be provided for memory cache\n");
				return NULL;
			} else if (!strncmp(value, "-", 1)) {
				ast_log(LOG_ERROR, "An invalid negative value '%s' has been provided for object stale lifetime for memory cache\n",
					value);
				return NULL;
			} else if (sscanf(value, "%30u", &cache->object_lifetime_stale) != 1) {
