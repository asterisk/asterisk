...
static int rtp_red_buffer(struct ast_rtp_instance *instance, struct ast_frame *frame)
{
	...
	if (frame->datalen > 0) {
		if (red->t140.datalen > 0) {
			...
			* buffer and sent regularly by a timer.  It can happen that commands such
			* as a backspace, CR, or LF get merged with regular text. This breaks some
			* UAs. In recent tests it could not be reproduced, but its probably better
			* to keep it.
			*/
