...
static int generate_forward_body(struct ast_str **bodytext, struct ast_sip_device_feature_sync_data *sync_data, enum forward_type fwdtype, const char *forward_to)
{
	...
	doc = ast_xml_new();
	if (!doc) {
		ast_debug(1, "Could not create new XML document for ForwardingEvent update\n");
		return -1;
	}
...
cleanup:
	ast_xml_close(doc);
	ast_debug(1, "Could not create new XML root node for ForwardingEvent update\n");
	return -1;
}
...
static int features_generate_body_content(void *body, void *data)
{
	...
	if (sync_data->update_needed_dnd) {
		...
		doc = ast_xml_new();
		if (!doc) {
			ast_debug(1, "Could not create new XML document for Do Not Disturb update\n");
			return -1;
		}
		...
		if (!root) {
			ast_xml_close(doc);
			ast_debug(1, "Could not create new XML root node for Do Not Disturb event\n");
			return -1;
		}
		...
		ast_xml_close(doc);
		if (len <= 0) {
			ast_debug(1, "XML document has length %d?\n", len);
		}
		if (buf) {
			ast_str_append(bodytext, 0, "%s", buf);
			ast_xml_free_text(buf);
