...
int ast_sip_session_create_invite(struct ast_sip_session *session, pjsip_tx_data **tdata)
{
	...
	if (session->endpoint->id.trust_outbound || !restricted) {
		ast_sip_modify_id_header(session->inv_session->dlg->pool,
			session->inv_session->dlg->local.info, &session->saved_connected_id);
	}

	if (!ast_strlen_zero(session->endpoint->fromuser)) {
		id_name_addr->display.ptr = NULL;
		id_name_addr->display.slen = 0;
		pj_strdup2(session->inv_session->dlg->pool, &id_uri->user, session->endpoint->fromuser);
	}

	if (!ast_strlen_zero(session->endpoint->fromdomain)) {
		pj_strdup2(session->inv_session->dlg->pool, &id_uri->host, session->endpoint->fromdomain);
	}

	ast_sip_add_usereqphone(session->endpoint, session->inv_session->dlg->pool,
		session->inv_session->dlg->local.info->uri);

	/* We need to save off From for RPID/PAI generation (for domain) */
	session->saved_from_hdr = pjsip_hdr_clone(session->inv_session->dlg->pool, session->inv_session->dlg->local.info);

	/* In chan_sip, fromuser and fromdomain trump restricted so we do the same thing */
	if (restricted) {
		pj_strdup2(session->inv_session->dlg->pool, &id_name_addr->display, "Anonymous");
		if (ast_strlen_zero(session->endpoint->fromuser)) {
			pj_strdup2(session->inv_session->dlg->pool, &id_uri->user, "anonymous");
		}
	}

	if (restricted && ast_strlen_zero(session->endpoint->fromdomain)) {
		pj_strdup2(session->inv_session->dlg->pool, &id_uri->host, "anonymous.invalid");
	}

	/*
	 * Now that dlg->local.info is set/anonymized correctly, we can create
	 * the invite message.  All future message should also have the correct From.
	 */
