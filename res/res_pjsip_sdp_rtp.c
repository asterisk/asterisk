	...
	struct ast_format *format_parsed)
{
	...
	if (!session || !session_media || !format_parsed) {
		ast_log(LOG_ERROR, "Could not initialize RED codec support needed for real-time text on endpoint %s\n",
			ast_sorcery_object_get_id(session->endpoint));
		return;
	}
				      ...
				      struct pjmedia_sdp_session *sdp, const struct pjmedia_sdp_session *remote, struct ast_stream *stream)
{
	...
	for (index = 0; index < ast_format_cap_count(caps); ++index) {
		...
		if (ast_format_get_type(format) == AST_MEDIA_TYPE_TEXT && strcasecmp(ast_format_get_name(format), "T140") == 0) {
			...
			ast_debug(3, "Got t140 payload %d for endpoint %s\n", t140_payload, ast_sorcery_object_get_id(session->endpoint));
		}
	}
