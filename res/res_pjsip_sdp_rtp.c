...
/*! \brief Check whether RTP is being received on any RTP instance in the session or not */
static int rtp_check_timeout(void *data)
{
	struct ast_sip_session_media_state *media_state;
	int i;
	int now;
	int timeout = 0;
	int timeout_factor = 1000;
	int lowest_elapsed = 0;
	...
	/* Check if the session is terminated or not */
	...
	for (i = 0; i < AST_VECTOR_SIZE(&media_state->sessions); i++) {
		...
		cur_timeout = ast_rtp_instance_get_timeout(rtp);
		/* This likely means the stream is on hold; we should schedule again, but keep
		 * checking the other RTP instances to see if there is a lower timeout value to use.
		 */
		if (cur_timeout <= 0) {
			schedule_again = 1;
			continue;
		 ...
		 * state. If it isn't UP, use half the timeout as the current
		 * timeout value to check for smallest timeout to avoid possible races
		 * and faster reacting to cases when there is no RTP at all.
		 */
		if (ast_channel_state(session->channel) != AST_STATE_UP || !ast_sockaddr_isnull(&session_media->direct_media_addr)) {
			/* Avoiding immediate disconnect after channel up or direct media has been stopped */
			ast_rtp_instance_set_last_rx(rtp, now);
			schedule_again = 1;

			/* If no timeout or this timeout is less than the lowest timeout so far, use it */
			if (!timeout || cur_timeout * 500 < timeout * timeout_factor) {
				timeout = cur_timeout;
				timeout_factor = 500;
			}
		} else {
			...
			if (elapsed < timeout) {
				int timeout_remaining = cur_timeout - elapsed;
				schedule_again = 1;

				/* If no timeout or this timeout is less than the lowest timeout so far, use it.
				 * We only do this when RTP has actually been received within the timeout window,
				 * otherwise we would get some crazy numbers for the timeout if, for instance,
				 * another stream was on hold and this one had not received RTP for a while
				 */
				if (!timeout || timeout_remaining * 1000 < timeout * timeout_factor) {
					timeout = timeout_remaining;
					timeout_factor = 1000;
				}
			} else {
				/* Update lowest elapsed time for disconnect log message below */
				if (!lowest_elapsed || elapsed < lowest_elapsed) {
	...
	/* If we didn't get a timeout value greater than 0, use the lowest non-zero timeout
	 * value between rtp.timeout and rtp.timeout_hold
	 */
	if (timeout <= 0) {
		if (session->endpoint->media.rtp.timeout > 0 && session->endpoint->media.rtp.timeout_hold > 0) {
			timeout = session->endpoint->media.rtp.timeout > session->endpoint->media.rtp.timeout_hold ?
				session->endpoint->media.rtp.timeout : session->endpoint->media.rtp.timeout_hold;
		} else if (session->endpoint->media.rtp.timeout > 0) {
			timeout = session->endpoint->media.rtp.timeout;
		} else {
			timeout = session->endpoint->media.rtp.timeout_hold;
		}

		/* If, for some reason, neither RTP timeout value is greater than 0, we can't reschedule */
		if (timeout <= 0) {
			schedule_again = 0;
		}
	}
	...
	ast_log(LOG_NOTICE, "Disconnecting channel '%s' for lack of RTP activity in %d seconds\n",
		ast_channel_name(session->channel), lowest_elapsed);
	...
	const struct pjmedia_sdp_session *remote, int index, struct ast_stream *asterisk_stream)
{
	 ...
	 * instance itself. The callback will automatically pick the lowest value across ALL
	 * RTP instances on the session and use that to reschedule.
	 */
	...
	if (!session->rtp_timeout_sched_task) {
		if (session->endpoint->media.rtp.timeout > 0 && session->endpoint->media.rtp.timeout_hold > 0) {
			rtp_timeout = session->endpoint->media.rtp.timeout > session->endpoint->media.rtp.timeout_hold ?
				session->endpoint->media.rtp.timeout : session->endpoint->media.rtp.timeout_hold;
		} else if (session->endpoint->media.rtp.timeout > 0) {
		...
		if (rtp_timeout > 0) {
			session->rtp_timeout_sched_task = ast_sip_schedule_task(session->serializer,
				rtp_timeout*1000, rtp_check_timeout, NULL,
				session, AST_SIP_SCHED_TASK_VARIABLE | AST_SIP_SCHED_TASK_DATA_AO2);
			if (!session->rtp_timeout_sched_task) {
				ast_log(LOG_ERROR, "Unable to schedule RTP timeout for channel '%s'\n",
					ast_channel_name(session->channel));
				SCOPE_EXIT_RTN_VALUE(-1, "Failed to schedule RTP timeout task\n");
			}
		}
	}
