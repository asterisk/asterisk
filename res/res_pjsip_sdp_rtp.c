	...
	struct ast_format *format_parsed)
{
	...
	if (ast_format_cap_has_type(session->endpoint->media.codecs, AST_MEDIA_TYPE_TEXT)) {
		if(strncasecmp(ast_format_get_name(format_parsed), "RED", 3) == 0){
				      ...
				      struct pjmedia_sdp_session *sdp, const struct pjmedia_sdp_session *remote, struct ast_stream *stream)
{
	...
	for (index = 0; index < ast_format_cap_count(caps); ++index) {
		...
		if (ast_format_get_type(format) == AST_MEDIA_TYPE_TEXT && strncasecmp(ast_format_get_name(format), "T140", 3) == 0) {
	...
	for (index = 0; index < ast_format_cap_count(caps); ++index) {
		...
		if (ast_format_get_type(format) == AST_MEDIA_TYPE_TEXT && strncasecmp(ast_format_get_name(format), "RED", 3) == 0) {
			if (*((int *)ast_format_attribute_get(format, "red_payload")) != t140_payload) {
				char tmp[10];
