...
/* This module provides the ast_sip_session_add_stored_pjsip_headers optional API */
...
/*** DOCUMENTATION
	<function name="PJSIP_HEADER" language="en_US">
		...
		<description>
			...
			<note><para>Headers added on a channel are also included on the outgoing
			REFER request generated when the <emphasis>Transfer()</emphasis> application
			is executed on that channel.</para></note>
			<example title="Add a header to the REFER generated by Transfer()">
			same => n,Set(PJSIP_HEADER(add,X-Transfer-Reason)=overflow)
			same => n,Transfer(PJSIP/${EXTEN}@endpoint)
			</example>
		</description>
	</function>
	<function name="PJSIP_INHERITABLE_HEADER" language="en_US">
		...
		<description>
			...
			REFER request generated when the <emphasis>Transfer()</emphasis> application
			is executed on that channel.</para></note>
			<example title="Add an inheritable header to the REFER generated by Transfer()">
			same => n,Set(PJSIP_INHERITABLE_HEADER(add,X-Transfer-Reason)=overflow)
			same => n,Transfer(PJSIP/${EXTEN}@endpoint)
			</example>
		</description>
	</function>
...
struct hdr_list_entry {
	pjsip_hdr *hdr;
	/*! Non-zero if the header was explicitly added via PJSIP_HEADER(add, ...).
	 * Zero if the header was captured from an incoming request for
	 * PJSIP_HEADER(read, ...). Only added headers may be applied to
	 * outbound messages. */
	int added;
...
/*!
 * \internal
 * \brief Add headers stored on the session by PJSIP_HEADER(add, ...) to an outgoing message
 *
 * Retrieve the header_datastore from the session.
 * Add each header in the list that was explicitly added via PJSIP_HEADER(add, ...)
 * to the outgoing message. Headers captured from incoming requests for
 * PJSIP_HEADER(read, ...) are never applied to outbound messages.
 * Remove the datastore afterwards so the headers are only applied once.
 * If no explicitly added headers are found the datastore is left untouched.
 *
 * These pjsip_hdr structures will have been created by add_header.
 * Because this may be called more than once with the same header
 * list (as in the case of an authentication exchange), each pjsip_hdr structure
 * MUST be newly cloned for each outgoing message.
 *
 * Exported as an optional API so that requests built outside of the session
 * supplement path (such as the REFER sent by the Transfer() application) can
 * include the stored headers as well.
 *
 * \param session The session to retrieve stored headers from
 * \param tdata The message to add the headers to
 *
 * \retval >=0 The number of headers added
 * \retval -1 No stored headers were found
 */
int AST_OPTIONAL_API_NAME(ast_sip_session_add_stored_pjsip_headers)(struct ast_sip_session *session,
	pjsip_tx_data *tdata)
{
	...
	AST_LIST_TRAVERSE(hdr_list, le, nextptr) {
		/* Skip headers captured from incoming requests for
		 * PJSIP_HEADER(read, ...); they must never be copied to
		 * outbound messages. */
		if (!le->added) {
			continue;
		}
	...
	if (!header_count) {
		/* Nothing explicitly added; leave the datastore alone as it
		 * may hold headers captured for PJSIP_HEADER(read, ...). */
		return -1;
	}

	ast_debug(3, "Added %d stored header(s) to channel %s\n", header_count,
		session->channel ? ast_channel_name(session->channel) : "(none)");
	/* Remove datastore after use */
...
/*!
 * \internal
 * \brief Add inheritable headers stored on the session's channel to an outgoing message
 *
 * Retrieve the inheritable_header_datastore from the session's channel, which
 * holds headers added via PJSIP_INHERITABLE_HEADER(add, ...), and add each one
 * to the outgoing message. Headers are prepended to the ast_variable list as
 * they are added, so the list is duplicated and reversed to apply them in
 * insertion order (the same reason outgoing_request() uses
 * ast_variables_reverse()).
 *
 * Unlike ast_sip_session_add_stored_pjsip_headers(), the channel datastore is
 * NOT consumed: inheritable headers must survive on the channel so they can
 * still be inherited by future channels.
 *
 * Exported as an optional API so that requests built outside of the session
 * supplement path (such as the REFER sent by the Transfer() application, which
 * the INVITE-only outgoing_request() supplement never handles on an
 * established dialog) can include the inheritable headers as well.
 *
 ...
 */
int AST_OPTIONAL_API_NAME(ast_sip_session_add_inheritable_pjsip_headers)(struct ast_sip_session *session,
	pjsip_tx_data *tdata)
{
	struct ast_datastore *chan_datastore;
	...
	/* Copy the header list out from under the channel lock; the channel
	 * datastore itself is left in place so the headers remain inheritable
	 * by future channels. */
	...
	headers = ast_variables_reverse(ast_variables_dup(chan_datastore->data));
	ast_channel_unlock(session->channel);

	if (!headers) {
		ast_log(LOG_ERROR, "Failed to duplicate inheritable header list for channel %s\n",
			session->channel ? ast_channel_name(session->channel) : "(none)");
		return -1;
	}
	...
	ast_debug(3, "Added %d inheritable header(s) to channel %s\n", header_count,
		session->channel ? ast_channel_name(session->channel) : "(none)");
...
static void outgoing_request(struct ast_sip_session *session, pjsip_tx_data * tdata)
{
	...
	if (session_datastore && session_datastore->data) {
		int added = ast_sip_session_add_stored_pjsip_headers(session, tdata);
