...
{
	return ast_variables_dup((struct ast_variable *) data);
}
...
{
	ast_variables_destroy((struct ast_variable *) data);
}
...
static int add_inheritable_header(struct ast_channel *channel, const char *header_name, const char *header_value)
{
	...
	if (!chan_datastore) {
		...
		if (chan_datastore) {
			...
			ast_debug(3, "Created new inheritable channel datastore for channel %s\n",
				ast_channel_name(channel));
		} else {
			ast_log(LOG_ERROR, "Failed to allocate channel datastore for channel %s\n",
				ast_channel_name(channel));
		...
		if (!chan_datastore->inheritance) {
			chan_datastore->inheritance = DATASTORE_INHERIT_FOREVER;
			ast_debug(3, "Upgraded channel datastore to inheritable for channel %s\n",
				ast_channel_name(channel));
		}
	}
	...
	if (!new_var) {
		ast_log(LOG_ERROR, "Failed to allocate variable for channel %s\n",
			ast_channel_name(channel));
		ast_channel_unlock(channel);
		return -1;
	}
