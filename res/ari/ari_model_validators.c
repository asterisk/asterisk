...
int ast_ari_validate_channel_json_received(struct ast_json *json)
{
	...
	for (iter = ast_json_object_iter(json); iter; iter = ast_json_object_iter_next(json, iter)) {
		if (strcmp("asterisk_id", ast_json_object_iter_key(iter)) == 0) {
			...
			prop_is_valid = ast_ari_validate_string(
				ast_json_object_iter_value(iter));
			if (!prop_is_valid) {
				ast_log(LOG_ERROR, "ARI ChannelJsonReceived field asterisk_id failed validation\n");
				res = 0;
			}
		} else
		if (strcmp("type", ast_json_object_iter_key(iter)) == 0) {
			...
			prop_is_valid = ast_ari_validate_string(
				ast_json_object_iter_value(iter));
			if (!prop_is_valid) {
				ast_log(LOG_ERROR, "ARI ChannelJsonReceived field type failed validation\n");
				res = 0;
			}
		} else
		if (strcmp("application", ast_json_object_iter_key(iter)) == 0) {
			...
			prop_is_valid = ast_ari_validate_string(
				ast_json_object_iter_value(iter));
			if (!prop_is_valid) {
				ast_log(LOG_ERROR, "ARI ChannelJsonReceived field application failed validation\n");
				res = 0;
			}
		} else
		if (strcmp("timestamp", ast_json_object_iter_key(iter)) == 0) {
			...
			prop_is_valid = ast_ari_validate_date(
				ast_json_object_iter_value(iter));
			if (!prop_is_valid) {
				ast_log(LOG_ERROR, "ARI ChannelJsonReceived field timestamp failed validation\n");
				res = 0;
			}
		} else
		if (strcmp("channel", ast_json_object_iter_key(iter)) == 0) {
			...
			prop_is_valid = ast_ari_validate_channel(
				ast_json_object_iter_value(iter));
			if (!prop_is_valid) {
				ast_log(LOG_ERROR, "ARI ChannelJsonReceived field channel failed validation\n");
				res = 0;
			}
		} else
		if (strcmp("data", ast_json_object_iter_key(iter)) == 0) {
			...
			prop_is_valid = ast_ari_validate_string(
				ast_json_object_iter_value(iter));
			if (!prop_is_valid) {
				ast_log(LOG_ERROR, "ARI ChannelJsonReceived field digit failed validation\n");
				res = 0;
			}
		} else
		{
			ast_log(LOG_ERROR,
				"ARI ChannelJsonReceived has undocumented field %s\n",
				ast_json_object_iter_key(iter));
			res = 0;
		}
	}
	...
	if (!has_type) {
		ast_log(LOG_ERROR, "ARI ChannelJsonReceived missing required field type\n");
		res = 0;
	}

	if (!has_application) {
		ast_log(LOG_ERROR, "ARI ChannelJsonReceived missing required field application\n");
		res = 0;
	}

	if (!has_timestamp) {
		ast_log(LOG_ERROR, "ARI ChannelJsonReceived missing required field timestamp\n");
		res = 0;
	}

	if (!has_channel) {
		ast_log(LOG_ERROR, "ARI ChannelJsonReceived missing required field channel\n");
		res = 0;
	}

	if (!has_data) {
		ast_log(LOG_ERROR, "ARI ChannelJsonReceived missing required field data\n");
		res = 0;
	}
...
{
	return ast_ari_validate_channel_json_received;
}
...
int ast_ari_validate_event(struct ast_json *json)
{
	...
	if (strcmp("ChannelJsonReceived", discriminator) == 0) {
		return ast_ari_validate_channel_json_received(json);
	} else
