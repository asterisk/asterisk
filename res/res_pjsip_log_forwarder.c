...
AST_MUTEX_DEFINE_STATIC(show_info_lock);

struct pjsip_show_info {
	pthread_t thread;
	int fd;
};

static struct pjsip_show_info show_info = {
...
static char *handle_pjsip_show_info(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a)
{
	...
	ast_mutex_lock(&show_info_lock);

	show_info.thread = pthread_self();
	show_info.fd = a->fd;
	pj_dump_config();
	show_info.fd = -1;
	show_info.thread = AST_PTHREADT_NULL;

	ast_mutex_unlock(&show_info_lock);
