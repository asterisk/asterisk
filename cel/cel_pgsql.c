...
static int process_my_load_module(struct ast_config *cfg)
{
	...
	if (PQstatus(conn) != CONNECTION_BAD) {
		...
		if (version >= PGSQL_MIN_VERSION_SCHEMA) {
			char *tmp_schemaname, *tmp_tablename;
			char *tablename, *schemaname;
			int lenschema = 0;
			if ((tmp_tablename = strrchr(table, '.'))) {
				lenschema = strlen(table) - strlen(tmp_tablename);
				tmp_schemaname = ast_alloca(lenschema * 2 + 1);
			...
			PQescapeStringConn(conn, schemaname, tmp_schemaname, lenschema, NULL);
			...
			"AND (a.attnum > 0) AND c.relname = '%s' ORDER BY c.relname, attnum", table);
		}
		/* Query the columns */
