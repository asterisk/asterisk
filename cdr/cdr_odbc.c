...
static SQLHSTMT execute_cb(struct odbc_obj *obj, void *data)
{
	...
	SQLRETURN ODBC_res;
	char sqlcmd[2048] = "", timestr[128], new_fields[120] = "", new_values[7] = "";
	struct ast_tm tm;
	SQLHSTMT stmt;
	int i = 0;
	...
	if (ast_test_flag(&config, CONFIG_NEWCDRFIELDS)) {
		snprintf(new_fields, sizeof(new_fields), "%s" ,",peeraccount,linkedid,sequence");
	...
	if (ast_test_flag(&config, CONFIG_LOGUNIQUEID)) {
		...
		"VALUES ({ts '%s'},?,?,?,?,?,?,?,?,?,?,?,?,?,?,? %s)",table, new_fields, timestr, new_values);
	} else {
		snprintf(sqlcmd,sizeof(sqlcmd),"INSERT INTO %s "
...
static int odbc_load_module(int reload)
{
	...
	do {
		...
		if (((tmp = ast_variable_retrieve(cfg, "global", "newcdrfields"))) && ast_true(tmp)) {
			ast_set_flag(&config, CONFIG_NEWCDRFIELDS);
			ast_debug(1, "cdr_odbc: Add new cdr fields\n");
		} else {
			ast_clear_flag(&config, CONFIG_NEWCDRFIELDS);
